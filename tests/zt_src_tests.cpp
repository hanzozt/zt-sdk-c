...
TEST_CASE("httpbin.zt:zt_src", "[integ]") {
    ...
    auto handler = [](zt_context ztx, const zt_event_t *ev) {
        auto t = (source_test *) zt_app_ctx(ztx);
        switch (ev->type) {
            ...
            case ZitiServiceEvent:
                ...
                zt_src_init(t->loop, &t->src, nullptr, ztx);
                tlsuv_http_init_with_src(t->loop, &t->clt, "http://httpbin.zt", &t->src);
    ...
    REQUIRE(zt_load_config(&zfg, cfg) == ZITI_OK);
    REQUIRE(zt_context_init(&ztx, &zfg) == ZITI_OK);
    REQUIRE(zt_context_set_options(ztx, &zopts) == ZITI_OK);
    REQUIRE(zt_context_run(ztx, test.loop) == ZITI_OK);
