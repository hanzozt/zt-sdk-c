...
public:
    ...
    void testRunStarting(Catch::TestRunInfo const &) override {
        ...
        if (id) {
            ...
            if (err != ZITI_OK) {
                fprintf(stderr, "failed to load zt context from %s: %d/%s\n", id, err, zt_errorstr(err));
                exit(1);
            }
        }
    }
    ...
    static zt_handle_t ztx() {
        return _ztx;
    }
};

zt_handle_t testRunListener::_ztx;

CATCH_REGISTER_LISTENER(testRunListener)
using namespace Catch::Matchers;

TEST_CASE("httpbin.zt", "[ztlib]") {
    zt_socket_t sock = Ziti_socket(SOCK_STREAM);
    REQUIRE(Ziti_connect_addr(sock, "httpbin.zt", 80) == 0);
