...
TEST_CASE("posture response response", "[model]") {
    ...
    zt_pr_response pr_resp;
    int rc = parse_zt_pr_response(&pr_resp, json, (int) strlen(json));
    REQUIRE(rc > 0);
    ...
    const char *session_json = R"(
{
  ...
  "type": "Dial"
    ...
    zt_session *s;
    REQUIRE(parse_zt_session_ptr(&s, session_json, (int) strlen(session_json)) == strlen(session_json));

    REQUIRE(model_list_size(&s->edge_routers) == 3);

    auto it = model_list_iterator(&s->edge_routers);
    auto er = (zt_edge_router *) model_list_it_element(it);
    ...
    er = (zt_edge_router *) model_list_it_element(it);
...
TEST_CASE("parse-services-array", "[model]") {
    ...
    zt_service **services;
    int rc = parse_zt_service_array(&services, json, (int) strlen(json));
    ...
    zt_posture_query_array pq_arr = nullptr;
    ...
    CHECK(services[2]->posture_query_set[0]->posture_queries[0]->query_type == zt_posture_query_type_PC_OS);
    ...
    CHECK(services[2]->posture_query_set[0]->posture_queries[1]->query_type == zt_posture_query_type_PC_Process);
    ...
    CHECK(services[2]->posture_query_set[0]->posture_queries[2]->query_type == zt_posture_query_type_PC_Domain);
    ...
    CHECK(services[2]->posture_query_set[0]->posture_queries[3]->query_type == zt_posture_query_type_PC_MAC);
...
TEST_CASE("parse-api-session", "[model]") {
    ...
    int rc = parse_zt_api_session_ptr(&session, json, (int) strlen(json));
    REQUIRE(rc > 0);
...
TEST_CASE("parse-error", "[model]") {
    ...
    zt_error err;
    int rc = parse_zt_error(&err, json, (int) strlen(json));
    REQUIRE(rc > 0);
...
TEST_CASE("parse-enrollment-jwt", "[model]") {
    const char *json = "{\n"
                       "\"em\":\"ott\",\n"
                       "\"exp\":1573411752,\n"
                       "\"iss\":\"https://demo.zt.netfoundry.io:1080\",\n"
                       "\"jti\":\"f581d770-fffc-11e9-a81a-000d3a1b4b17\",\n"
                       "\"sub\":\"c17291f4-37fe-4cdb-9f57-3eb757b648f5\"\n"
                       "}";

    zt_enrollment_jwt ej;
    int rc = parse_zt_enrollment_jwt(&ej, json, (int) strlen(json));
    CHECK(ej.method == zt_enrollment_methods.ott);
    CHECK_THAT(ej.controller, Equals("https://demo.zt.netfoundry.io:1080"));
...
TEST_CASE("test service array", "[model]") {
    const char *json = R"([
  {
    ...
    zt_service_array arr;
    REQUIRE(parse_zt_service_array(&arr, json, strlen(json)) == strlen(json));
    REQUIRE(arr != nullptr);
    CHECK(arr[1] == nullptr);
    CHECK_THAT(arr[0]->name, Equals("httpbin"));

    free_zt_service_array(&arr);
}
...
TEST_CASE("service config test", "[model]") {
    ...
    zt_service s;
    REQUIRE(parse_zt_service(&s, j, strlen(j)) > 0);

    {
        zt_client_cfg_v1 cfg;
        REQUIRE(zt_service_get_config(&s, "zt-tunneler-client.v1", &cfg,
                                        (int (*)(void *, const char *, size_t)) (parse_zt_client_cfg_v1)) == 0);

        CHECK(cfg.hostname.type == zt_address_hostname);
        CHECK_THAT(cfg.hostname.addr.hostname, Equals("hello.zt"));
        CHECK(cfg.port == 80);

        zt_intercept_cfg_v1 intercept;
        zt_intercept_from_client_cfg(&intercept, &cfg);
        model_list_iter it = model_list_iterator(&intercept.protocols);
        CHECK(*(zt_protocol *) model_list_it_element(it) == zt_protocols.tcp);
        it = model_list_it_next(it);
        CHECK(*(zt_protocol *) model_list_it_element(it) == zt_protocols.udp);
        CHECK(model_list_it_next(it) == nullptr);

        auto pr = (zt_port_range *) model_list_head(&intercept.port_ranges);
        CHECK(pr->high == 80);
        CHECK(pr->low == 80);
        CHECK(model_list_size(&intercept.port_ranges) == 1);

        auto addr = (zt_address *) model_list_head(&intercept.addresses);
        CHECK(addr->type == zt_address_hostname);
        CHECK_THAT(addr->addr.hostname, Equals("hello.zt"));
        CHECK(model_list_size(&intercept.addresses) == 1);

        free_zt_client_cfg_v1(&cfg);
        free_zt_intercept_cfg_v1(&intercept);
    }

    {
        zt_intercept_cfg_v1 cfg;
        REQUIRE(zt_service_get_config(&s, "intercept.v1", &cfg,
                                        (int (*)(void *, const char *, size_t)) (parse_zt_intercept_cfg_v1)) == 0);

        CHECK(*(zt_protocol *) model_list_head(&cfg.protocols) == zt_protocols.tcp);
        zt_address ip1_2_3_4;
        parse_zt_address_str(&ip1_2_3_4, "1.2.3.4");
        CHECK(zt_address_match((zt_address *) model_list_head(&cfg.addresses), &ip1_2_3_4) == 0);
        auto pr = (zt_port_range *) model_list_head(&cfg.port_ranges);
        CHECK(pr->high == 80);
        free_zt_intercept_cfg_v1(&cfg);
    }
...
TEST_CASE("config change", "[model]") {
    ...
    zt_service s, s1, s2;
    REQUIRE(parse_zt_service(&s, j1, strlen(j1)) > 0);
    REQUIRE(parse_zt_service(&s1, j1, strlen(j1)) > 0);
    REQUIRE(parse_zt_service(&s2, j2, strlen(j2)) > 0);

    CHECK(cmp_zt_service(&s, &s1) == 0);
    CHECK(cmp_zt_service(&s, &s2) != 0);
    CHECK(cmp_zt_service(&s2, &s1) != 0);
...
TEST_CASE("parse-ctrl-version", "[model]") {
    ...
    zt_version ver;
    REQUIRE(parse_zt_version(&ver, json, strlen(json)) > 0);
    ...
    CHECK(*ver.capabilities[0] == zt_ctrl_cap_HA_CONTROLLER);
    CHECK(*ver.capabilities[1] == zt_ctrl_cap_OIDC_AUTH);
...
TEST_CASE("parse-zt-address", "[model]") {
    const char *j = R"("foo.bar")";
    zt_address addr;

    int rc = parse_zt_address(&addr, j, strlen(j));
    CHECK(rc > 0);
    CHECK(addr.type == zt_address_hostname);
    CHECK_THAT(addr.addr.hostname, Catch::Matchers::Equals("foo.bar"));
}

TEST_CASE("parse-zt-intercept1", "[model]") {
    const char *json = R"( {
    ...
    char addr_str[64];
    zt_intercept_cfg_v1 intercept;
    int len = parse_zt_intercept_cfg_v1(&intercept, json, strlen(json));
    REQUIRE(len > 0);
    REQUIRE(model_list_size(&intercept.addresses) == 4);

    int idx = 0;
    model_list_iter it = model_list_iterator(&intercept.addresses);
    auto *addr = (zt_address *) model_list_it_element(it);
    CHECK(addr->type == zt_address_hostname);
    CHECK_THAT(addr->addr.hostname, Catch::Matchers::Equals("foo.bar"));

    it = model_list_it_next(it);
    addr = (zt_address *) model_list_it_element(it);
    CHECK(addr->type == zt_address_cidr);
    ...
    auto json_out = zt_intercept_cfg_v1_to_json(&intercept, MODEL_JSON_COMPACT, nullptr);
    free_zt_intercept_cfg_v1(&intercept);
    free(json_out);
}

TEST_CASE("zt-address-match", "[model]") {

    const char *json = R"( {
        "addresses": ["foo.bar", "*.zt", "*.yahoo.com", "1.1.1.1", "100.64.0.0/10", "ff::1/64"]
    })";
    ...
    CHECK(zt_addrstr_match_list("foo.bar", &intercept.addresses) == 0);
    CHECK(zt_addrstr_match_list("foo.baz", &intercept.addresses) == -1);
    CHECK(zt_addrstr_match_list("AWESOME.ZITI", &intercept.addresses) > 0);
    CHECK(zt_addrstr_match_list("Yahoo.COM", &intercept.addresses) >= 0);
    CHECK(zt_addrstr_match_list("1.1.1.1", &intercept.addresses) == 0);
    CHECK(zt_addrstr_match_list("1.1.1.2", &intercept.addresses) == -1);
    CHECK(zt_addrstr_match_list("100.127.1.1", &intercept.addresses) == 22); // match 100.64.0.0/10
    CHECK(zt_addrstr_match_list("100.128.1.2", &intercept.addresses) == -1);
    CHECK(zt_addrstr_match_list("ff::abcd:1", &intercept.addresses) > 0);
    CHECK(zt_addrstr_match_list("ff:abcd::1", &intercept.addresses) == -1);

    free_zt_intercept_cfg_v1(&intercept);
}

TEST_CASE("zt-intercept_test", "[model]") {
    zt_client_cfg_v1 cltV1;
    REQUIRE(parse_zt_address_str(&cltV1.hostname, "httpbin.zt") == 0);
    cltV1.port = 80;

    zt_intercept_cfg_v1 interceptCfgV1;
    REQUIRE(zt_intercept_from_client_cfg(&interceptCfgV1, &cltV1) == 0);
    REQUIRE(model_list_size(&interceptCfgV1.addresses) == 1);

    free_zt_intercept_cfg_v1(&interceptCfgV1);
    free_zt_client_cfg_v1(&cltV1);
}
...
TEST_CASE("load cfg", "[model]") {
  ...
  "id": {
    ...
    zt_config cfg;
    REQUIRE(zt_load_config(&cfg, nullptr) == ZITI_INVALID_CONFIG);

    REQUIRE(zt_load_config(&cfg, good_json) == ZITI_OK);
    free_zt_config(&cfg);

    REQUIRE(zt_load_config(&cfg, bad_json) == ZITI_CONFIG_NOT_FOUND);
    free_zt_config(&cfg);

    REQUIRE(zt_load_config(&cfg, non_identity_json) == ZITI_INVALID_CONFIG);
    free_zt_config(&cfg);
}