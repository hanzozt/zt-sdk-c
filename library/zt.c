...
static void version_pre_auth_cb(const zt_version *version, const zt_error *err, void *ctx);
static void update_ctrl_status(zt_context ztx, int code, const char *msg);

static void edge_routers_cb(zt_edge_router_array ers, const zt_error *err, void *ctx);

static void zt_init_async(zt_context ztx, void *data);

static void zt_re_auth(zt_context ztx);

static void ztx_prepare(uv_prepare_t *prep);
static void grim_reaper(zt_context ztx);

static void ztx_work_async(zt_context ztx);

static void zt_stop_internal(zt_context ztx, void *data);

static void zt_start_internal(zt_context ztx, void *init_req);

static void set_service_posture_policy_map(zt_service *service);

static void shutdown_and_free(zt_context ztx);

static void ca_bundle_cb(char *pkcs7, const zt_error *err, void *ctx);

static void update_identity_data(zt_identity_data *data, const zt_error *err, void *ctx);

static void on_create_cert(zt_create_api_cert_resp *resp, const zt_error *e, void *ctx);

static int ztx_init_controller(zt_context ztx);
static void ztx_config_update(zt_context ztx);

static void api_session_cb(zt_api_session *, const zt_error *, void *);
...
static zt_options default_options = {
        .disabled = false,
...
static int init_tls_from_config(tls_context *tls, zt_config *cfg, struct tls_credentials *creds) {
    PREP(zt);
    ...
    TRY(zt, load_key_internal(tls, &pk, cfg->id.key));

    tlsuv_certificate_t c = NULL;
    if (cfg->id.cert) {
        const char *cert;
        size_t cert_len = parse_ref(cfg->id.cert, &cert);
        TRY(zt, tls->load_cert(&c, cert, cert_len));
    }
    TRY(zt, tls->set_own_cert(tls, pk, c));
...
int load_tls(zt_config *cfg, tls_context **ctx, struct tls_credentials *creds) {

    // load ca from zt config if present
    const char *ca;
...
int zt_set_client_cert(zt_context ztx, const char *cert_buf, size_t cert_len, const char *key_buf, size_t key_len) {
    tlsuv_private_key_t pk;
...
extern void zt_set_enabled(zt_context ztx, bool enabled) {
    ...
    zt_queue_work(ztx, enabled ? zt_start_internal : zt_stop_internal, NULL);
}

void zt_set_auth_started(zt_context ztx) {
    ZTX_LOG(DEBUG, "setting api_session_state[%d] to %d", ztx->auth_state, ZitiAuthStateAuthStarted);
    FREE(ztx->session_token);
}

void zt_set_unauthenticated(zt_context ztx, const zt_error *err) {
    if (err) {
    ...
    model_map_clear(&ztx->sessions, (void (*)(void *)) free_zt_session_ptr);

    zt_ctrl_clear_auth(ztx_get_controller(ztx));

    // disconnect all channels since ER may not enforce session expiration
    const char *k;
    zt_channel_t *ch;
    MODEL_MAP_FOREACH(k, ch, &ztx->channels) {
        zt_channel_disconnect(ch, ZITI_NOT_AUTHORIZED);
    }
...
void zt_set_impossible_to_authenticate(zt_context ztx, const zt_error *err) {
    zt_controller *ctrl = ztx_get_controller(ztx);
    ...
    FREE(ztx->session_token);
    zt_ctrl_clear_auth(ctrl);
    zt_send_event(ztx, &(zt_event_t){
        .type = ZitiContextEvent,
        .ctx = (struct zt_context_event){
...
void zt_set_partially_authenticated(zt_context ztx, const zt_auth_query_mfa *mfa_q) {
    ...
    switch (mfa_q->type_id) {
        ...
        case zt_auth_query_type_EXT_JWT: {
            ...
            if (signer == NULL) {
                ...
                ev.auth.action = zt_auth_cannot_continue;
                ev.auth.detail = "signer not found";
            } else {
                ztx_init_external_auth(ztx, signer);
                ev.auth.action = zt_auth_login_external;
                ev.auth.detail = signer->name;
            }
            break;
        }
        case zt_auth_query_type_Unknown:
        default:
            ev.auth.action = zt_auth_cannot_continue;
...
static void ctrl_list_cb(zt_controller_detail_array ctrls, const zt_error *err, void *ctx) {
    zt_context ztx = ctx;
    if (err) {
    ...
    for (int i = 0; ctrls[i] != NULL; i++) {
        zt_controller_detail *detail = ctrls[i];
        ...
        if (edge_api && edge_api->url) {
            ...
            free_zt_controller_detail_ptr(detail);
        }
    }
    ...
    model_map_clear(&old_details, (void (*)(void *)) free_zt_controller_detail_ptr);
    free(ctrls);
}

void zt_set_fully_authenticated(zt_context ztx, const char *session_token) {
    ...
    if (ztx->auth_method->kind == OIDC) {
        zt_ctrl_clear_auth(ctrl);
        zt_ctrl_set_token(ctrl, session_token);
        if (ctrl->is_ha) {
            zt_ctrl_list_controllers(ctrl, ctrl_list_cb, ztx);
        }

        const char* er_name;
        zt_channel_t *ch;
        MODEL_MAP_FOREACH(er_name, ch, &ztx->channels) {
            zt_channel_update_token(ch, session_token);
        }
    }
    zt_ctrl_get_well_known_certs(ctrl, ca_bundle_cb, ztx);
    zt_ctrl_current_api_session(ctrl, api_session_cb, ztx);
    zt_ctrl_current_identity(ctrl, update_identity_data, ztx);
    ...
    if (ztx->id_creds.cert == NULL && ztx->session_creds.cert == NULL) {
        ...
        snprintf(common_name, sizeof(common_name), "zt-%u-%" PRIu64,
        ...
        if (rc != 0) {
            ...
            zt_ctrl_create_api_certificate(ztx_get_controller(ztx), ztx->sessionCsr, on_create_cert, ztx);
        }
    }

    zt_services_refresh(ztx, true);
    zt_posture_init(ztx, 20);
}
...
static void zt_stop_internal(zt_context ztx, void *data) {
    if (ztx->enabled) {
        ...
        if (ztx->posture_checks) {
            zt_posture_checks_free(ztx->posture_checks);
        ...
        // close all channels
        zt_close_channels(ztx, ZITI_DISABLED);

        FREE(ztx->last_update);
        zt_event_t ev = {0};
        ev.type = ZitiServiceEvent;
        ev.service.removed = calloc(model_map_size(&ztx->services) + 1, sizeof(zt_service *));
        int idx = 0;
        ...
        zt_send_event(ztx, &ev);
        free_zt_service_array(&ev.service.removed);

        zt_ctrl_cancel(ztx_get_controller(ztx));
        zt_set_unauthenticated(ztx, NULL);
        update_ctrl_status(ztx, ZITI_DISABLED, zt_errorstr(ZITI_DISABLED));
        ztx->enabled = false;
...
static void zt_start_internal(zt_context ztx, void *init_req) {
    if (!ztx->enabled) {
        ...
        if (rc != 0) {
            ZTX_LOG(ERROR, "invalid TLS config: %s", zt_errorstr(rc));
            zt_event_t ev = {
                    .type = ZitiContextEvent,
                    .ctx = {
                        .ctrl_status = rc,
...
static void on_ctrl_list_change(zt_context ztx, const model_map *endpoints) {
...
static int ztx_init_controller(zt_context ztx) {
    ...
    int rc = zt_ctrl_init(ztx->loop, &ztx->ctrl, &ztx->config.controllers, ztx->tlsCtx);
    if (rc != 0) {
    ...
    ZTX_LOG(INFO, "Loading zt context with controller[%s]", ztx_controller(ztx));
    zt_ctrl_set_callbacks(ztx_get_controller(ztx), ztx, on_ctrl_redirect,
                            (zt_ctrl_change_cb) on_ctrl_list_change);
    if (ztx->opts.api_page_size != 0) {
        zt_ctrl_set_page_size(ztx_get_controller(ztx), ztx->opts.api_page_size);
    }
    return 0;
}
...
static void ext_jwt_singers_cb(zt_jwt_signer_array signers, const zt_error *err, void *ctx) {
    struct ztx_req_s *req = ctx;

    struct zt_ctx *ztx = req->ztx;
    zt_ext_signers_cb cb = (zt_ext_signers_cb) req->cb;
    if (err) {
        ZTX_LOG(WARN, "failed to get external auth providers: %s", err->message);
        CALL_CB(cb, ztx, (int)err->err, NULL, req->cb_ctx);
    } else {
        model_map_clear(&ztx->ext_signers, (void (*)(void *)) free_zt_jwt_signer_ptr);
        zt_jwt_signer *s;
        FOR(s, signers) {
...
int zt_get_ext_jwt_signers(zt_context ztx, zt_ext_signers_cb cb, void *ctx) {
    NEWP(req, struct ztx_req_s);
    req->ztx = ztx;
    req->cb = cb;
    req->cb_ctx = ctx;

    zt_ctrl_list_ext_jwt_signers(ztx_get_controller(ztx), ext_jwt_singers_cb, req);
    return ZITI_OK;
}

int zt_use_ext_jwt_signer(zt_context ztx, const char *name) {
    if (ztx == NULL || name == NULL) {
        return ZITI_INVALID_STATE;
    }

    const zt_jwt_signer *signer = model_map_get(&ztx->ext_signers, name);
    if (signer == NULL) {
...
void *zt_app_ctx(zt_context ztx) {
    return ztx->opts.app_ctx;
}

void zt_set_app_ctx(zt_context ztx, void *app_ctx) {
    ztx->opts.app_ctx = app_ctx;
}

const char *zt_get_controller(zt_context ztx) {
    return ztx_get_controller(ztx)->url;
}

const zt_version *zt_get_controller_version(zt_context ztx) {
    return &ztx_get_controller(ztx)->version;
}

const zt_identity *zt_get_identity(zt_context ztx) {
    if (ztx->identity_data) {
        return (const zt_identity *) ztx->identity_data;
    }

    return NULL;
}

int zt_get_transfer_rates(zt_context ztx, double *up, double *down) {
...
static void free_ztx(uv_handle_t *h) {
    ...
    model_map_clear(&ztx->ext_signers, (_free_f)free_zt_jwt_signer_ptr);
    model_map_clear(&ztx->ctrl_details, (_free_f) free_zt_controller_detail_ptr);
    zt_auth_query_free(ztx->auth_queries);
    zt_posture_checks_free(ztx->posture_checks);
    model_map_clear(&ztx->services, (_free_f) free_zt_service_ptr);
    model_map_clear(&ztx->sessions, (_free_f) free_zt_session_ptr);
    zt_set_unauthenticated(ztx, NULL);
    FREE(ztx->last_update);
    FREE(ztx->session_token);

    zt_ctrl_close(ztx_get_controller(ztx));
    ...
    free_zt_config(&ztx->config);

    zt_event_t ev = {0};
    ev.type = ZitiContextEvent;
    ev.ctx.ctrl_status = ZITI_DISABLED;
    ev.ctx.err = zt_errorstr(ZITI_DISABLED);

    zt_send_event(ztx, &ev);
    free_zt_identity_data_ptr(ztx->identity_data);
...
int zt_shutdown(zt_context ztx) {
    ZTX_LOG(INFO, "Ziti is shutting down");
    ztx->closing = true;

    zt_queue_work(ztx, zt_stop_internal, NULL);
...
int zt_get_appdata(zt_context ztx, const char *key, void *data,
    ...
    const char *app_data_json = zt_get_appdata_raw(ztx, key);
...
void zt_dump(zt_context ztx, int (*printer)(void *arg, const char *fmt, ...), void *ctx) {
    ...
    const zt_posture_query_set *pq_set;
    const zt_posture_query *pq;
        ...
        MODEL_MAP_FOREACH(pq_set_id, pq_set, &zs->posture_query_map) {
            ...
            for (int idx = 0; pq_set->posture_queries[idx] != NULL; idx++) {
                ...
                printer(ctx, "\t\t\tquery_id[%s] type[%s] is_passing[%s] timeout[%d] timeoutRemaining[%d]\n",
                        pq->id, zt_posture_query_types.name(pq->query_type),
    ...
    MODEL_MAP_FOREACH(er_id, ch, &ztx->channels) {
        ...
        printer(ctx, "\tconnected[%c] version[%s] address[%s]",
                zt_channel_is_connected(ch) ? 'Y' : 'N', ch->version, ch->url);
        if (zt_channel_is_connected(ch)) {
            printer(ctx, " latency[%" PRIu64 "]\n", zt_channel_latency(ch));
        } else {
            printer(ctx, "\n");
        }
    }
    ...
    MODEL_MAP_FOREACH(id, conn, &ztx->connections) {
        ...
        if (conn->type == Transport && conn->parent == NULL) {
            printer(ctx, "conn[%d/%s]: state[%s] service[%s] using ch[%d/%s]\n",
                    conn->conn_id, conn->marker, zt_conn_state(conn), conn->service,
        ...
        if (conn->type == Server) {
            ...
            while (it != NULL) {
                ...
                zt_connection child = model_map_it_value(it);
                printer(ctx, "\tchild[%d/%s]: state[%s] caller_id[%s] ch[%d/%s]\n",
                        child_id, child->marker, zt_conn_state(child), zt_conn_source_identity(child),
...
int zt_conn_init(zt_context ztx, zt_connection *conn, void *data) {
    struct zt_ctx *ctx = ztx;
    NEWP(c, struct zt_conn);
    c->zt_ctx = ztx;
    c->data = data;
...
void *zt_conn_data(zt_connection conn) {
    return conn != NULL ? conn->data : NULL;
}

void zt_conn_set_data(zt_connection conn, void *data) {
    if (conn != NULL) {
        conn->data = data;
    }
}

const char *zt_conn_source_identity(zt_connection conn) {
    return conn != NULL ? conn->source_identity : NULL;
}


void zt_send_event(zt_context ztx, const zt_event_t *e) {
    if (ztx->opts.events & e->type) {
        CALL_CB(ztx->opts.event_cb, ztx, e);
    }
}

void ztx_config_update(zt_context ztx) {
    zt_send_event(ztx, &(zt_event_t){
...
static void set_service_flags(zt_service *s) {
    for (int i = 0; s->permissions[i] != NULL; i++) {
        if (*s->permissions[i] == zt_session_types.Dial) {
            s->perm_flags |= ZITI_CAN_DIAL;
        }
        if (*s->permissions[i] == zt_session_types.Bind) {
            s->perm_flags |= ZITI_CAN_BIND;
        }
    }
}

static void service_cb(zt_service *s, const zt_error *err, void *ctx) {
    struct ztx_req_s *req = ctx;
    int rc = ZITI_SERVICE_UNAVAILABLE;

    if (s != NULL) {
        set_service_flags(s);
        zt_service *old = model_map_set(&req->ztx->services, s->name, s);
        free_zt_service_ptr(old);
        rc = ZITI_OK;
    } else {
        if (err) {
            rc = (int)err->err;
    ...
    CALL_CB((zt_service_cb)req->cb, req->ztx, s, rc, req->cb_ctx);
    free(req);
}

int zt_service_available(zt_context ztx, const char *service, zt_service_cb cb, void *ctx) {
    if (!ztx->enabled) return ZITI_DISABLED;

    zt_service *s = model_map_get(&ztx->services, service);
    if (s != NULL) {
    ...
    zt_ctrl_get_service(ztx_get_controller(ztx), service, service_cb, req);
    return ZITI_OK;
}

static void term_cb(zt_terminator_array terminators, const zt_error *err, void *ctx) {
    struct ztx_req_s *r = ctx;
    CALL_CB((zt_terminator_cb)r->cb, r->ztx, (const zt_terminator * const *) terminators,
            err ? err->err : ZITI_OK, r->cb_ctx);
    free_zt_terminator_array(&terminators);
    free(r);
}

static void term_srv_cb(zt_context ztx, const zt_service *s, int err, void *ctx) {
    struct ztx_req_s *req = ctx;
    if (err || s == NULL) {
        CALL_CB((zt_terminator_cb)req->cb, req->ztx, NULL,
                err ? err : ZITI_SERVICE_UNAVAILABLE , req->cb_ctx);
        free(req);
        return;
    }

    zt_ctrl_list_terminators(ztx_get_controller(req->ztx), s->id, term_cb, req);
}

int zt_list_terminators(zt_context ztx, const char *service, zt_terminator_cb cb, void *ctx) {
    NEWP(req, struct ztx_req_s);
    req->ztx = ztx;
    req->cb = cb;
    req->cb_ctx = ctx;

    return zt_service_available(ztx, service, term_srv_cb, req);
}

const zt_service *zt_service_for_addr_str(zt_context ztx, zt_protocol proto, const char *addr, int port) {
    zt_address a;
    if (parse_zt_address_str(&a, addr) != -1) {
        return zt_service_for_addr(ztx, proto, &a, port);
    }
    ZTX_LOG(WARN, "invalid address host[%s]", addr);
    return NULL;
}

const zt_service *zt_service_for_addr(zt_context ztx, zt_protocol proto, const zt_address *addr, int port) {
    int best_score = -1;
    zt_service *best = NULL;

    zt_service *srv;
    const char *name;
    MODEL_MAP_FOREACH(name, srv, &ztx->services) {
        zt_intercept_cfg_v1 intercept = {0};
        zt_client_cfg_v1 clt_cfg = {0};
        if (zt_service_get_config(srv, ZITI_INTERCEPT_CFG_V1, &intercept, (parse_service_cfg_f) parse_zt_intercept_cfg_v1) == ZITI_OK ||
                (zt_service_get_config(srv, ZITI_CLIENT_CFG_V1, &clt_cfg, (parse_service_cfg_f) parse_zt_client_cfg_v1) == ZITI_OK &&
                 zt_intercept_from_client_cfg(&intercept, &clt_cfg) == ZITI_OK
                )
        ) {
            int match = zt_intercept_match2(&intercept, proto, addr, port);
...
int zt_listen(zt_connection serv_conn, const char *service, zt_listen_cb lcb, zt_client_cb cb) {
    return zt_bind(serv_conn, service, NULL, lcb, cb);
}

int zt_listen_with_options(zt_connection serv_conn, const char *service, zt_listen_opts *listen_opts,
                             zt_listen_cb lcb, zt_client_cb cb) {
    return zt_bind(serv_conn, service, listen_opts, lcb, cb);
}

/**
 * `zt_re_auth` attempts to re-authenticate with the controller.
...
static void zt_re_auth(zt_context ztx) {
    ...
    zt_get_ext_jwt_signers(ztx, NULL, NULL);

    // always get controller version to get the right auth method
    zt_ctrl_get_version(ztx_get_controller(ztx), version_pre_auth_cb, ztx);
}

static void set_posture_query_defaults(zt_service *service) {
    int posture_set_idx;
...
static bool service_posture_check_timeouts_changed(zt_context ztx, const zt_posture_query_set *new_set, const zt_posture_query_set *old_set) {
...
static int is_service_updated(zt_context ztx, zt_service *new, zt_service *old) {
    ...
    const zt_posture_query_set *new_set;
    MODEL_MAP_FOREACH(policy_id, new_set, &new->posture_query_map) {
        zt_posture_query_set *old_set = model_map_get(&old->posture_query_map, policy_id);
    ...
    const zt_posture_query_set *old_set;
    MODEL_MAP_FOREACH(policy_id, old_set, &old->posture_query_map) {
        zt_posture_query_set *new_set_by_old_id = model_map_get(&new->posture_query_map, policy_id);
...
static void update_services(zt_service_array services, const zt_error *error, void *ctx) {
    zt_context ztx = ctx;

    // schedule next refresh
    zt_services_refresh(ztx, false);
    ...
    zt_event_t ev = {
            ...
            .service = {
                    .removed = calloc(current_size + 1, sizeof(zt_service *)),
                    .changed = calloc(current_size + 1, sizeof(zt_service *)),
                    .added = calloc(idx + 1, sizeof(zt_service *)),
            }
    };
    ...
    model_map_iter it = model_map_iterator(&ztx->services);
    while (it != NULL) {
        zt_service *updt = model_map_remove(&updates, model_map_it_key(it));
        ...
        if (updt != NULL) {
            ...
            zt_invalidate_session(ztx, s->id, zt_session_types.Dial);
            it = model_map_it_remove(it);
        }
    }
    ...
    for (idx = 0; ev.service.changed[idx] != NULL; idx++) {
        ...
        zt_service *old = model_map_set(&ztx->services, s->name, s);
        zt_invalidate_session(ztx, old ? old->id : NULL, zt_session_types.Dial);
        free_zt_service_ptr(old);
...
static void set_service_posture_policy_map(zt_service *service) {
    ...
    //free the array so that subsequent zt_service_free() calls do not double free on the map and array
    FREE(service->posture_query_set);
}

static void check_service_update(zt_service_update *update, const zt_error *err, void *ctx) {
    zt_context ztx = ctx;
    ...
    if (err) {
        ...
        zt_ctrl_get_services(ztx_get_controller(ztx), update_services, ztx);

    } else {
        ZTX_LOG(VERBOSE, "not updating: last_update is same previous (%s == %s)", update->last_change,
                ztx->last_update);
        free_zt_service_update(update);
        zt_services_refresh(ztx, false);
    }
    FREE(update);
}
...
static void refresh_cb(void *data) {
    ...
    zt_ctrl_current_identity(ztx_get_controller(ztx), update_identity_data, ztx);
    zt_ctrl_current_edge_routers(ztx_get_controller(ztx), edge_routers_cb, ztx);
    zt_ctrl_get_services_update(ztx_get_controller(ztx), check_service_update, ztx);
}
...
static void edge_routers_cb(zt_edge_router_array ers, const zt_error *err, void *ctx) {
    ...
    zt_edge_router **erp = ers;
    while (*erp) {
        zt_edge_router *er = *erp;

        // check if it is already in the list
        if (model_map_remove(&curr_routers, er->name) == NULL) {
            if (zt_channel_connect(ztx, er) == ZITI_OK) {
                ers_changed = true;
            ...
            zt_channel_set_url(ch, er->protocols.tls);
        }

        free_zt_edge_router(er);
        free(er);
        erp++;
    }
    free(ers);
    ...
    while (it != NULL) {
        ...
        zt_channel_close(ch, ZITI_GATEWAY_UNAVAILABLE);
        it = model_map_it_remove(it);
        ers_changed = true;
    }
...
static void update_identity_data(zt_identity_data *data, const zt_error *err, void *ctx) {
    ...
    if (err) {
        ...
        if (err->err == ZITI_AUTHENTICATION_FAILED) {
            ZTX_LOG(WARN, "api session is no longer valid. Trying to re-auth");
            zt_set_unauthenticated(ztx, err);
        ...
        if (data != NULL) {
            ...
            update_ctrl_status(ztx, ZITI_OK, zt_errorstr(ZITI_OK));
        }
    }
}
...
static void on_create_cert(zt_create_api_cert_resp *resp, const zt_error *e, void *ctx) {
    ...
    if (e) {
        ...
        free_zt_create_api_cert_resp_ptr(resp);
    }
    FREE(ztx->sessionCsr);
}
...
static void update_ctrl_status(zt_context ztx, int errCode, const char *errMsg) {
...
void zt_invalidate_session(zt_context ztx, const char *service_id, zt_session_type type) {
    if (type == zt_session_types.Dial && service_id != NULL) {
        zt_session *s = model_map_remove(&ztx->sessions, service_id);
        free_zt_session_ptr(s);
    }
}

static const zt_version sdk_version = {
        .version = to_str(ZITI_VERSION),
        .revision = to_str(ZITI_COMMIT),
        .build_date = __DATE__ " " __TIME__,
};

const zt_version *zt_get_version() {
    return &sdk_version;
}
...
static void grim_reaper(zt_context ztx) {
    ...
    while (it != NULL) {
        zt_connection conn = model_map_it_value(it);
...
static void ztx_prep_deadlines(zt_context ztx) {
    if (LIST_EMPTY(&ztx->deadlines)) {
...
void ztx_prepare(uv_prepare_t *prep) {
    ...
    if (ztx->session_token && ztx->session_expiration.tv_sec > 0) {
        ...
        if (ztx->session_expiration.tv_sec < now.tv_sec) {
            ...
            zt_set_unauthenticated(ztx, &error);
            zt_force_api_session_refresh(ztx);
        }
    }
    ...
    // NOTE: stalled zt connections are flushed with idle handlers,
...
void zt_on_channel_event(zt_channel_t *ch, zt_router_status status, int err, zt_context ztx) {
    ...
    if (status == EdgeRouterDisconnected && err == ZITI_CONNABORT) {
        ...
        zt_force_api_session_refresh(ch->ztx);
    }
...
static void copy_oidc(zt_context ztx, const zt_jwt_signer *oidc) {
    if (oidc == NULL) return;
...
int zt_context_init(zt_context *ztx, const zt_config *config) {
    if (config == NULL ||
    ...
    zt_context ctx = calloc(1, sizeof(*ctx));
    ctx->id = ++ztx_seq;
...
int zt_context_set_options(zt_context ztx, const zt_options *options) {
    if (options == NULL) {
...
int zt_context_run(zt_context ztx, uv_loop_t *loop) {
    if (ztx->loop) {
        return ZITI_INVALID_STATE;
    }
    ...
    zt_queue_work(ztx, zt_init_async, NULL);

    return ZITI_OK;
}

int zt_refresh(zt_context ztx) {
    if (!ztx->enabled) return ZITI_DISABLED;
...
static void version_pre_auth_cb(const zt_version *version, const zt_error *err, void *ctx) {
    ...
    bool use_oidc = zt_has_capability(version, zt_ctrl_caps.OIDC_AUTH);
    ...
    // make sure zt_ctrl client is configured for correct auth
    zt_ctrl_set_legacy(ztx_get_controller(ztx), !use_oidc);
...
void ztx_auth_state_cb(void *ctx, zt_auth_state state, const void *data) {
    zt_context ztx = ctx;
    switch (state) {
        case ZitiAuthStateUnauthenticated:
            zt_set_unauthenticated(ztx, (const zt_error *) data);
            break;
        case ZitiAuthStateAuthStarted:
            zt_set_auth_started(ztx);
            break;
        case ZitiAuthStatePartiallyAuthenticated: {
            zt_set_partially_authenticated(ztx, data);
            break;
        }
        case ZitiAuthStateFullyAuthenticated:
            zt_set_fully_authenticated(ztx, data);
            break;
        case ZitiAuthImpossibleToAuthenticate:
            zt_set_impossible_to_authenticate(ztx, (const zt_error*)data);
            break;
    }
...
zt_channel_t * ztx_get_channel(zt_context ztx, const zt_edge_router *er) {
    ...
    zt_channel_t *ch = (zt_channel_t *) model_map_get(&ztx->channels, er->name);
    if (ch == NULL) {
        zt_channel_connect(ztx, er);
    }
    return ch;
}

struct cert_ext_req {
    zt_api_session *session;
    zt_context ztx;
    zt_extend_cert_authenticator_resp *cert_resp;
    tlsuv_certificate_t new_cert;
};

static void cert_verify_cb(void *r, const zt_error *err, void *ctx) {
    struct cert_ext_req *req = ctx;
    zt_context ztx = req->ztx;
    if (err) {
    ...
    free_zt_api_session_ptr(req->session);
    free_zt_extend_cert_authenticator_resp_ptr(req->cert_resp);
    free(req);
}

static void cert_extend_cb(zt_extend_cert_authenticator_resp *resp, const zt_error *err, void *ctx) {
    struct cert_ext_req *req = ctx;
    zt_context ztx = req->ztx;
    if (err) {
    ...
    if (ztx->tlsCtx->load_cert(&req->new_cert, resp->client_cert_pem, strlen(resp->client_cert_pem)) != 0) {
        ...
        free_zt_extend_cert_authenticator_resp_ptr(resp);
        free_zt_api_session_ptr(req->session);
        free(req);
        return;
    }
    ...
    req->cert_resp = resp;
    zt_ctrl_verify_extend_cert_authenticator(
            ztx_get_controller(ztx), req->session->authenticator_id,
            resp->client_cert_pem, cert_verify_cb, req);

}

static void api_session_cb(zt_api_session *api_sess, const zt_error *err, void *ctx) {
    zt_context ztx = ctx;
    char *csr = NULL;
    size_t len = 0;
    if (api_sess) {
        ...
        while(it) {
            zt_session *s = model_map_it_value(it);
        ...
        api_sess = NULL;
        zt_ctrl_extend_cert_authenticator(ztx_get_controller(ztx),
                                            ext_req->session->authenticator_id, csr,
                                            cert_extend_cb, ext_req);
    }

    done:
    free_zt_api_session_ptr(api_sess);
    free(csr);
}
