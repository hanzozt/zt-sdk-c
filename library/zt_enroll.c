...
struct zt_enroll_req {
    zt_enroll_opts opts;

    zt_enroll_cb enroll_cb;
    void *ctx;

    zt_enrollment_jwt_header jwt_header;
    zt_enrollment_jwt enrollment;
    char *sig;
    size_t sig_len;
    ...
    zt_controller controller;
};

static int fetch_network_token(struct zt_enroll_req * er);

static int start_enrollment(struct zt_enroll_req * er);


static void well_known_certs_cb(char *base64_encoded_pkcs7, const zt_error *err, void *req);

static void enroll_cb(zt_enrollment_resp *resp, const zt_error *err, void *ctx);

static void free_enroll_req(struct zt_enroll_req *er);

int verify_controller_jwt(const struct tlsuv_certificate_s *cert, void *ctx) {
    ZITI_LOG(DEBUG, "verifying JWT signature");

    struct zt_enroll_req *er = ctx;
    enum hash_algo md;
...
int zt_enroll(const zt_enroll_opts *opts, uv_loop_t *loop,
                zt_enroll_cb enroll_cb, void *enroll_ctx) {
...
static void network_jwt_cb(zt_network_jwt_array arr, const zt_error *err, void *ctx) {
    struct zt_enroll_req *er = ctx;
    if (err) {
    ...
    free_zt_network_jwt_array(&arr);

    int rc = start_enrollment(er);
    if (rc != ZITI_OK) {
        er->enroll_cb(NULL, rc, zt_errorstr(rc), er->ctx);
        free_enroll_req(er);
    }
}

int fetch_network_token(struct zt_enroll_req *er) {
    ...
    if (url.query != NULL) {
        ...
        rc = zt_ctrl_init(er->loop, &er->controller, &ctrls, er->tls);
        if (rc == ZITI_OK) {
            zt_ctrl_get_network_jwt(&er->controller, network_jwt_cb, er);
        }
    }
...
static int start_enrollment(struct zt_enroll_req *er) {
    ...
    if (er->opts.key != NULL) {
        if (load_key_internal(er->tls, &er->pk, er->opts.key) != 0) {
            ...
            if (rc != 0) {
                ZITI_LOG(ERROR, "failed to load or generate pkcs11 key: %s", zt_errorstr(rc));
                return ZITI_KEY_LOAD_FAILED;
            }
        }
    }
    ...
    if (er->enrollment.method == zt_enrollment_method_ottca ||
        er->enrollment.method == zt_enrollment_method_ca) {
        if (er->pk == NULL) {
    ...
    zt_ctrl_init(er->loop, &er->controller, &er->cfg.controllers, er->tls);

    zt_ctrl_get_well_known_certs(&er->controller, well_known_certs_cb, er);
    return ZITI_OK;
}
...
static void free_enroll_req(struct zt_enroll_req * er) {
    if (er) {
        ...
        free_zt_enrollment_jwt_header(&er->jwt_header);
        free_zt_enrollment_jwt(&er->enrollment);
        free(er->sig);
        free_zt_config(&er->cfg);
...
static void complete_request(struct zt_enroll_req *er, int err) {
    if (err == ZITI_OK) {
        er->enroll_cb(&er->cfg, err, NULL, er->ctx);
    } else {
        er->enroll_cb(NULL, err, zt_errorstr(err), er->ctx);
    }

    free_enroll_req(er);
}

static void enroll_network(struct zt_enroll_req *er) {
    complete_request(er, ZITI_OK);
}

static void enroll_ott(struct zt_enroll_req *er) {
    size_t len;
    int rc = 0;
    if (er->opts.key == NULL) {
        if (er->opts.use_keychain && er->tls->generate_keychain_key) {
            ...
            string_buf_fmt(keyname_buf, "keychain:zt://%s@%.*s:%d",
            ...
            if (rc != 0) {
                ...
                ZITI_LOG(DEBUG, "failed to generate keychain key[%s], trying to load", zt_errorstr(rc));
    ...
    zt_ctrl_enroll(&er->controller, er->enrollment.method, er->enrollment.token, csr, er->opts.name, enroll_cb, er);
    free(csr);
}

static void enroll_ca(struct zt_enroll_req *er) {
    assert(er->pk != NULL);
    assert(er->cert != NULL);
    ...
    zt_ctrl_enroll(&er->controller, er->enrollment.method, er->enrollment.token, NULL, er->opts.name, enroll_cb, er);
}

static void well_known_certs_cb(char *base64_encoded_pkcs7, const zt_error *err, void *req) {
    struct zt_enroll_req *er = req;
    if (err != NULL) {
    ...
    case zt_enrollment_method_network:
        enroll_network(er);
        break;
    case zt_enrollment_method_ott:
        enroll_ott(er);
        break;
    case zt_enrollment_method_ottca:
    case zt_enrollment_method_ca:
        enroll_ca(er);
        break;
    case zt_enrollment_method_Unknown:
    default:
...
static void enroll_cb(zt_enrollment_resp *resp, const zt_error *err, void *enroll_ctx) {
    assert(enroll_ctx);
    struct zt_enroll_req *er = enroll_ctx;
    ...
    complete_request(er, ZITI_OK);
    free_zt_enrollment_resp_ptr(resp);
}
...
int parse_enrollment_jwt(const char *token, zt_enrollment_jwt_header *zejh, zt_enrollment_jwt *zej, char **sig, size_t *sig_len) {
    ...
    if (parse_zt_enrollment_jwt_header(zejh, header, header_len) < 0) {
    ...
    if (parse_zt_enrollment_jwt(zej, body, blen) < 0) {
