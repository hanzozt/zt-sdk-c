...
const char *const ERROR_MSG_NO_API_SESSION_TOKEN = "no api session token set for zt_controller";
...
typedef void (*ctrl_cb_t)(void *, const zt_error *, ctrl_resp_t *);
typedef void (*ctrl_resp_cb_t)(void *, const zt_error *, void *);
...
static void internal_get_version(zt_controller *ctrl);

static struct ctrl_resp *prepare_resp(zt_controller *ctrl, ctrl_resp_cb_t cb, body_parse_fn parser, void *ctx);

static void ctrl_paging_req(struct ctrl_resp *resp);

static void ctrl_default_cb(void *s, const zt_error *e, struct ctrl_resp *resp);

static void ctrl_body_cb(tlsuv_http_req_t *req, char *b, ssize_t len);

static const char* ctrl_next_ep(zt_controller *ctrl, const char *current);
...
static void ctrl_resp_cb(tlsuv_http_resp_t *r, void *data) {
    ...
    if (r->code < 0) {
        ...
        // cancellation is cased by closing of zt context
        // do not log error
        if (r->code == UV_ECANCELED) {
            e = ZITI_DISABLED;
            code = zt_errorstr(ZITI_DISABLED);
        } else {
        ...
        const char *new_addr = find_header(r, "zt-ctrl-address");
        if (new_addr) {
            FREE(resp->new_address);
            resp->new_address = strdup(new_addr);
        }

        const char *instance_id = find_header(r, "zt-instance-id");
...
static void ctrl_default_cb(void *s, const zt_error *e, struct ctrl_resp *resp) {
    ...
    if (resp->new_address && strcmp(resp->new_address, ctrl->url) != 0) {
        ...
        if(detail == NULL) {
            detail = alloc_zt_controller_detail();
        }
        FREE(detail->name);
...
static void internal_ctrl_list_cb(zt_controller_detail_array arr, const zt_error *err, void *ctx) {
    zt_controller *ctrl = ctx;
    ...
    FOR (d, arr) {
        ...
        if (addr != NULL) {
            ...
            zt_controller_detail *old_detail = model_map_get(&ctrl->endpoints, addr->url);
            ...
            free_zt_controller_detail_ptr(d);
        }
    }
    ...
    if (model_map_size(&new_eps) == 0) {
        ...
        model_map_clear(&old, (void (*)(void *)) free_zt_controller_detail_ptr);
        if (ctrl->is_ha) {
            ctrl->change_cb(ctrl->cb_ctx, &ctrl->endpoints);
        }
    } else {
        CTRL_LOG(VERBOSE, "no ctrl list change");
        model_map_clear(&new_eps, (void (*)(void *)) free_zt_controller_detail_ptr);
    }
    free(arr);
}

static void internal_version_cb(zt_version *v, zt_error *e, struct ctrl_resp *resp) {
    zt_controller *ctrl = resp->ctrl;
    if (e) {
    ...
    if (v) {
        ...
        ctrl->is_ha = zt_has_capability(&ctrl->version, zt_ctrl_caps.HA_CONTROLLER);
...
void zt_ctrl_clear_auth(zt_controller *ctrl) {
    ...
    if (ctrl->client) {
        CTRL_LOG(DEBUG, "clearing api session token for zt_controller");
        tlsuv_http_header(ctrl->client, "zt-session", NULL);
        zt_ctrl_set_token(ctrl, NULL);
    }
}

static void ctrl_login_cb(zt_api_session *s, zt_error *e, struct ctrl_resp *resp) {
    zt_controller *ctrl = resp->ctrl;
    if (e) {
        CTRL_LOG(ERROR, "%s(%s)", e->code, e->message);
        zt_ctrl_clear_auth(resp->ctrl);
    }
...
static void ctrl_logout_cb(void *s, zt_error *e, struct ctrl_resp *resp) {
    zt_controller *ctrl = resp->ctrl;
...
static void ctrl_service_cb(zt_service **services, zt_error *e, struct ctrl_resp *resp) {
    zt_service *s = services != NULL ? services[0] : NULL;
    ctrl_default_cb(s, e, resp);
    free(services);
}
...
static void ctrl_body_cb(tlsuv_http_req_t *req, char *b, ssize_t len) {
    ...
    if (len > 0) {
        ...
        if (resp->resp_content == ctrl_content_text) {
            ...
            if (err_json) {
                if (zt_error_from_json(&error, err_json) != 0) {
                    error.code = strdup("INVALID_CONTROLLER_RESPONSE");
...
static const char* ctrl_next_ep(zt_controller *ctrl, const char *current) {
    ...
    zt_controller_detail *curr = current ?
...
int zt_ctrl_init(uv_loop_t *loop, zt_controller *ctrl, model_list *urls, tls_context *tls) {
    *ctrl = (zt_controller){0};
    if (model_list_size(urls) == 0) {
        ZITI_LOG(ERROR, "no zt controller endpoints");
    ...
    MODEL_LIST_FOREACH(ep, *urls) {
        zt_controller_detail *detail = alloc_zt_controller_detail();
    ...
    CTRL_LOG(DEBUG, "zt controller client initialized");
    internal_get_version(ctrl);
    return ZITI_OK;
}

int zt_ctrl_set_token(zt_controller *ctrl, const char *token) {
    if (token == NULL) {
    ...
    if (ctrl->is_ha) {
        zt_ctrl_list_controllers(ctrl, internal_ctrl_list_cb, ctrl);
    }
...
int zt_ctrl_close(zt_controller *ctrl) {
    free_zt_version(&ctrl->version);
    model_map_clear(&ctrl->endpoints, (void (*)(void *)) free_zt_controller_detail_ptr);
    FREE(ctrl->url);
...
static void internal_get_version(zt_controller *ctrl) {
    struct ctrl_resp *resp = MAKE_RESP(ctrl, NULL, zt_version_ptr_from_json, NULL);
...
void zt_ctrl_login(
    ...
    size_t body_len;
    char *body = zt_auth_req_to_json(&authreq, 0, &body_len);


    struct ctrl_resp *resp = MAKE_RESP(ctrl, cb, zt_api_session_ptr_from_json, ctx);
...
void zt_ctrl_login_ext_jwt(zt_controller *ctrl, const char *jwt,
    ...
    zt_auth_req authreq = {
            .sdk_info = {
                    .type = "zt-sdk-c",
                    .version = (char *) zt_get_build_version(0),
                    .revision = (char *) zt_git_commit(),
                    .branch = (char *) zt_git_branch(),
                    .app_id = (char *) APP_ID,
...
static bool verify_api_session(zt_controller *ctrl, ctrl_resp_cb_t cb, void *ctx) {
    if(!ctrl->has_token) {
...
void zt_ctrl_current_identity(zt_controller *ctrl, void(*cb)(zt_identity_data *, const zt_error *, void *), void *ctx) {
    if(!verify_api_session(ctrl, (void (*)(void *, const zt_error *, void *)) cb, ctx)) return;

    struct ctrl_resp *resp = MAKE_RESP(ctrl, cb, zt_identity_data_ptr_from_json, ctx);
...
void zt_ctrl_list_terminators(zt_controller *ctrl, const char *service_id,
                               void (*cb)(zt_terminator_array, const zt_error*, void *ctx), void *ctx) {
    if(!verify_api_session(ctrl, (ctrl_resp_cb_t) cb, ctx)) return;

    char path[512];
    snprintf(path, sizeof(path), "/services/%s/terminators", service_id);

    struct ctrl_resp *resp = MAKE_RESP(ctrl, cb, zt_terminator_array_from_json, ctx);
    resp->paging = true;
    resp->base_path = path;
    ctrl_paging_req(resp);
}
...
void zt_ctrl_list_controllers(zt_controller *ctrl,
    ...
    struct ctrl_resp *resp = MAKE_RESP(ctrl, cb, zt_controller_detail_array_from_json, ctx);
    resp->paging = true;
    resp->base_path = "/controllers";
    ctrl_paging_req(resp);
}

void zt_ctrl_list_ext_jwt_signers(
        zt_controller *ctrl,
        void (*cb)(zt_jwt_signer_array, const zt_error*, void *ctx), void *ctx) {
    struct ctrl_resp *resp = MAKE_RESP(ctrl, cb, zt_jwt_signer_array_from_json, ctx);
    resp->paging = true;
    resp->base_path = "/external-jwt-signers";
    ctrl_paging_req(resp);
}

void zt_ctrl_get_network_jwt(zt_controller *ctrl, void(*cb)(zt_network_jwt_array, const zt_error*, void *ctx), void *ctx) {
    struct ctrl_resp *resp = MAKE_RESP(ctrl, cb, zt_network_jwt_array_from_json, ctx);
    resp->paging = true;
...
void zt_ctrl_get_services_update(zt_controller *ctrl, void (*cb)(zt_service_update *, const zt_error *, void *), void *ctx) {
    if(!verify_api_session(ctrl, (void (*)(void *, const zt_error *, void *)) cb, ctx)) return;

    struct ctrl_resp *resp = MAKE_RESP(ctrl, cb, zt_service_update_ptr_from_json, ctx);
    start_request(ctrl->client, "GET", "/current-api-session/service-updates", ctrl_resp_cb, resp);
}

void zt_ctrl_get_services(zt_controller *ctrl, void (*cb)(zt_service_array, const zt_error *, void *), void *ctx) {
    if(!verify_api_session(ctrl, (void (*)(void *, const zt_error *, void *)) cb, ctx)) return;

    struct ctrl_resp *resp = MAKE_RESP(ctrl, cb, zt_service_array_from_json, ctx);
...
void zt_ctrl_current_edge_routers(zt_controller *ctrl, void (*cb)(zt_edge_router_array, const zt_error *, void *),
    ...
    struct ctrl_resp *resp = MAKE_RESP(ctrl, cb, zt_edge_router_array_from_json, ctx);
    resp->paging = true;
...
void
zt_ctrl_get_service(zt_controller *ctrl, const char *service_name, void (*cb)(zt_service *, const zt_error *, void *),
                      void *ctx) {
    if(!verify_api_session(ctrl, (void (*)(void *, const zt_error *, void *)) cb, ctx)) return;
...
void zt_ctrl_list_service_routers(zt_controller *ctrl, const zt_service *srv, routers_cb cb, void *ctx) {
    if(!verify_api_session(ctrl, (void (*)(void *, const zt_error *, void *)) cb, ctx)) return;

    struct ctrl_resp *resp = MAKE_RESP(ctrl, cb, zt_service_routers_ptr_from_json, ctx);
...
void zt_ctrl_get_session(
    ...
    struct ctrl_resp *resp = MAKE_RESP(ctrl, cb, zt_session_ptr_from_json, ctx);
...
void zt_ctrl_create_session(
    ...
    size_t len = snprintf(content, 128,
                          "{\"serviceId\": \"%s\", \"type\": \"%s\"}",
                          service_id, zt_session_types.name(type));
...
void zt_ctrl_get_sessions(
    ...
    struct ctrl_resp *resp = MAKE_RESP(ctrl, cb, zt_session_array_from_json, ctx);
    resp->paging = true;
    resp->base_path = "/sessions";
    ctrl_paging_req(resp);
}

static void enroll_pem_cb(void *body, const zt_error *err, struct ctrl_resp *resp) {
    zt_enrollment_resp *er = alloc_zt_enrollment_resp();
    er->cert = (char *) body;
    if (resp->resp_cb) {
...
void
zt_ctrl_enroll(zt_controller *ctrl, zt_enrollment_method method, const char *token, const char *csr,
                 const char *name,
                 void (*cb)(zt_enrollment_resp *, const zt_error *, void *),
                 void *ctx) {
    char *csr_copy = csr ? strdup(csr) : NULL;

    struct ctrl_resp *resp = MAKE_RESP(ctrl, cb, zt_enrollment_resp_ptr_from_json, ctx);

    tlsuv_http_req_t *req = start_request(ctrl->client, "POST", "/enroll", ctrl_enroll_http_cb, resp);
    size_t q_count = method == zt_enrollment_method_ca ? 1 : 2;
    const tlsuv_http_pair q_params[] = {
        { "method", zt_enrollment_methods.name(method)},
        { "token",  token },
    };
    ...
    if (csr_copy) {
        ...
        tlsuv_http_req_header(req, "Content-Type", "application/json");
        if (name != NULL) {
            zt_identity id = {.name = name};
            size_t body_len;
            char *body = zt_identity_to_json(&id, MODEL_JSON_COMPACT, &body_len);
            tlsuv_http_req_data(req, body, body_len, free_body_cb);
        }
    }
}
...
void
zt_ctrl_get_well_known_certs(zt_controller *ctrl, void (*cb)(char *, const zt_error *, void *), void *ctx) {
    struct ctrl_resp *resp = MAKE_RESP(ctrl, cb, NULL, ctx);
...
void zt_pr_post(zt_controller *ctrl, char *body, size_t body_len,
                  void(*cb)(zt_pr_response *, const zt_error *, void *), void *ctx) {
    if (!verify_api_session(ctrl, (ctrl_resp_cb_t) cb, ctx)) { return; }

    struct ctrl_resp *resp = MAKE_RESP(ctrl, cb, zt_pr_response_ptr_from_json, ctx);
...
void zt_pr_post_bulk(zt_controller *ctrl, char *body, size_t body_len,
                       void(*cb)(zt_pr_response *, const zt_error *, void *), void *ctx) {
    if (!verify_api_session(ctrl, (ctrl_resp_cb_t) cb, ctx)) { return; }
...
void zt_ctrl_get_mfa(zt_controller *ctrl, void(*cb)(zt_mfa_enrollment *, const zt_error *, void *), void *ctx) {
    ...
    struct ctrl_resp *resp = MAKE_RESP(ctrl, cb, zt_mfa_enrollment_ptr_from_json, ctx);
...
void zt_ctrl_get_mfa_recovery_codes(zt_controller *ctrl, char *code, void(*cb)(zt_mfa_recovery_codes *, const zt_error *, void *), void *ctx) {
    ...
    struct ctrl_resp *resp = MAKE_RESP(ctrl, cb, zt_mfa_recovery_codes_ptr_from_json, ctx);
...
void zt_ctrl_extend_cert_authenticator(zt_controller *ctrl, const char *authenticatorId, const char *csr, void(*cb)(zt_extend_cert_authenticator_resp*, const zt_error *, void *), void *ctx) {
    ...
    struct ctrl_resp *resp = MAKE_RESP(ctrl, cb, zt_extend_cert_authenticator_resp_ptr_from_json, ctx);

    char path[128];
    snprintf(path, sizeof(path), "/current-identity/authenticators/%s/extend", authenticatorId);

    zt_extend_cert_authenticator_req extend_req;
    extend_req.client_cert_csr = csr;

    size_t body_len;
    char *body = zt_extend_cert_authenticator_req_to_json(&extend_req, 0, &body_len);
...
void zt_ctrl_verify_extend_cert_authenticator(zt_controller *ctrl, const char *authenticatorId, const char *client_cert, void(*cb)(void *, const zt_error *, void *), void *ctx) {
    ...
    zt_verify_extend_cert_authenticator_req verify_req;
    verify_req.client_cert = client_cert;

    size_t body_len;
    char *body = zt_verify_extend_cert_authenticator_req_to_json(&verify_req, 0, &body_len);
...
void zt_ctrl_create_api_certificate(zt_controller *ctrl, const char *csr_pem,
                                      void(*cb)(zt_create_api_cert_resp *, const zt_error *, void *), void *ctx) {

    if(!verify_api_session(ctrl, (ctrl_resp_cb_t) cb, ctx)) return;

    struct ctrl_resp *resp = MAKE_RESP(ctrl, cb, zt_create_api_cert_resp_ptr_from_json, ctx);

    const char *path = "/current-api-session/certificates";

    zt_create_api_cert_req cert_req = {
            .client_cert_csr = (char*)csr_pem
    };

    size_t body_len;
    char *body = zt_create_api_cert_req_to_json(&cert_req, 0, &body_len);
