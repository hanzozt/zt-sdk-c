...
static int zt_src_connect(tlsuv_src_t *src, const char *, const char *, tlsuv_src_connect_cb cb, void *conn_ctx);

static void zt_src_release(tlsuv_src_t *src);
...
static void zlnf_conn_cb(zt_connection conn, int status);

static ssize_t zlnf_data_cb(zt_connection conn, const uint8_t *data, ssize_t length);

static void zlnf_write_cb(zt_connection conn, ssize_t status, void *ctx);
...
static const uv_link_methods_t zt_link_methods = {
...
int zt_src_init(uv_loop_t *l, tlsuv_src_t *st, const char *svc, zt_context ztx) {
    st->loop = l;
    st->connect = zt_src_connect;
    st->connect_cb = NULL;
    st->release = zt_src_release;
    st->link = malloc(sizeof(zt_link_t));
    uv_link_init(st->link, &zt_link_methods);

    zt_link_t *zl = (zt_link_t *) st->link;
    if (svc) {
...
static int
zt_src_connect(tlsuv_src_t *src, const char *host, const char *port, tlsuv_src_connect_cb cb, void *conn_ctx) {
    zt_link_t *zl = (zt_link_t *) src->link;

    zt_address a;
    parse_zt_address_str(&a, host);

    if (zl->service == NULL) { // find service by intercept
        int portnum = (int) strtol(port, NULL, 10);
        const zt_service *s = zt_service_for_addr(zl->ztx, zt_protocols.tcp, &a, portnum);
        if (s == NULL) {
    ...
    int status = zt_conn_init(zl->ztx, &zl->conn, src);
    if (status != ZITI_OK) {
        return status;
    }
    ...
    size_t app_data_len = snprintf(app_data, sizeof(app_data),
                                   ...
                                   "\"dst_port\":\"%s\""
                                   "}",
                                   a.type == zt_address_cidr ? "dst_ip" : "dst_hostname",
                                   host, port);
    ...
    return zt_dial_with_options(zl->conn, zl->service, &opts, zlnf_conn_cb, zlnf_data_cb);
}
...
static void zlnf_conn_cb(zt_connection conn, int status) {
    tlsuv_src_t *src = (tlsuv_src_t *) zt_conn_data(conn);
...
static ssize_t zlnf_data_cb(zt_connection conn, const uint8_t *data, ssize_t length) {
    ...
    if (length == ZITI_EOF) {
        ...
        ZITI_LOG(ERROR, "unexpected error: %s", zt_errorstr(length));
...
static int zl_write(uv_link_t *link, uv_link_t *source, 
    ...
    return zt_write(zl->conn, (uint8_t *) bufs[0].base, bufs[0].len, zlnf_write_cb, req);
}
...
void zl_close(uv_link_t* link, uv_link_t* source, uv_link_close_cb link_close_cb) {
    ...
    zt_close(zl->conn, NULL);
    link_close_cb((uv_link_t *) zl);
}

const char* zl_strerror(uv_link_t* link, int err) {
    return zt_errorstr(err);
}
