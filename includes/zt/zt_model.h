...
XX(target_token, zt_target_token, none, targetToken, __VA_ARGS__) \
...
XX(id, zt_id_cfg, none, id, __VA_ARGS__)           \
...
XX(capabilities, zt_ctrl_cap, array, capabilities, __VA_ARGS__) \
XX(api_versions, zt_api_versions, ptr, apiVersions, __VA_ARGS__)
...
XX(query_type, zt_posture_query_type, none, queryType, __VA_ARGS__) \
XX(process, zt_process, ptr, process, __VA_ARGS__) \
XX(processes, zt_process, array, processes, __VA_ARGS__) \
...
XX(posture_queries, zt_posture_query, array, postureQueries, __VA_ARGS__)

#define ZITI_SERVICE_MODEL(XX, ...)                                                          \
    XX(id, model_string, none, id, __VA_ARGS__)                                              \
    XX(name, model_string, none, name, __VA_ARGS__)                                          \
    XX(permissions, zt_session_type, array, permissions, __VA_ARGS__)                      \
    ...
    XX(posture_query_map, zt_posture_query_set, map, posturePolicies, __VA_ARGS__)         \
    XX(terminator_strategy, zt_terminator_strategy, none, terminatorStrategy, __VA_ARGS__) \
    XX(updated_at,model_string, none, updatedAt, __VA_ARGS__)
...
XX(hostname, zt_address, none, hostname, __VA_ARGS__) \
...
#define ZITI_CLIENT_CFG_V1 "zt-tunneler-client.v1"

#define ZITI_INTERCEPT_CFG_V1_MODEL(XX, ...) \
XX(protocols, zt_protocol, list, protocols, __VA_ARGS__) \
XX(addresses, zt_address, list, addresses, __VA_ARGS__) \
XX(port_ranges, zt_port_range, list, portRanges, __VA_ARGS__) \
XX(dial_options, tag, map, dialOptions, __VA_ARGS__) \
XX(source_ip, model_string, none, sourceIp, __VA_ARGS__) \
XX(allowed_source_addresses, zt_address, list, allowedSourceAddresses, __VA_ARGS__)
...
XX(type, zt_proxy_server_type, none, type, __VA_ARGS__)

#define ZITI_ADDRESS_TRANSLATION_MODEL(XX, ...) \
XX(from, zt_address, none, from, __VA_ARGS__) \
XX(to, zt_address, none, to, __VA_ARGS__)     \
...
XX(forward_address_translations, zt_address_translation, array, forwardAddressTranslations, __VA_ARGS__) \
XX(allowed_addresses, zt_address, array, allowedAddresses, __VA_ARGS__) \
XX(port, model_number, none, port, __VA_ARGS__) \
XX(forward_port, model_bool, none, forwardPort, __VA_ARGS__) \
XX(allowed_port_ranges, zt_port_range, array, allowedPortRanges, __VA_ARGS__) \
XX(allowed_source_addresses, zt_address, array, allowedSourceAddresses, __VA_ARGS__) \
XX(proxy, zt_proxy_server, none, proxy, __VA_ARGS__) \
XX(listen_options, zt_listen_options, ptr, listenOptions, __VA_ARGS__)

#define ZITI_HOST_CFG_V2_MODEL(XX, ...) \
XX(terminators, zt_host_cfg_v1, list, terminators, __VA_ARGS__)
...
enum zt_address_type {
    zt_address_hostname,
    zt_address_cidr
};

typedef struct zt_address_s {
    enum zt_address_type type;
    union {
        struct {
            char af;
            unsigned int bits;
...
// make sure zt model functions are properly exported
#ifdef MODEL_VISIBILITY
#undef MODEL_VISIBILITY
#endif
#define MODEL_VISIBILITY ZITI_FUNC

ZITI_FUNC int parse_zt_address_str(zt_address *addr, const char *addr_str);

ZITI_FUNC int zt_address_print(char *buf, size_t max, const zt_address *address);

ZITI_FUNC int zt_address_match(const zt_address *addr, const zt_address *range);

ZITI_FUNC int zt_address_match_s(const char *addr, const zt_address *range);

ZITI_FUNC int zt_addrstr_match_list(const char *addr, const model_list *range);
ZITI_FUNC int zt_address_match_list(const zt_address *addr, const model_list *range);

ZITI_FUNC int zt_address_match_array(const char *addr, zt_address **range);

DECLARE_MODEL_FUNCS(zt_address)

DECLARE_ENUM(zt_protocol, ZITI_PROTOCOL_ENUM)

ZITI_FUNC model_bool zt_protocol_match(zt_protocol proto, const model_list *proto_list);
ZITI_FUNC int zt_port_match(int port, const model_list *port_range_list);

DECLARE_ENUM(zt_session_type, ZITI_SESSION_TYPE_ENUM)

DECLARE_ENUM(zt_auth_query_type, ZITI_AUTH_QUERY_TYPE_ENUM)

DECLARE_ENUM(zt_posture_query_type, ZITI_POSTURE_QUERY_TYPE_ENUM)

DECLARE_ENUM(zt_ctrl_cap, ZITI_CTRL_CAP_ENUM)

DECLARE_ENUM(zt_target_token, ZITI_SIGNER_TARGET_TOKEN)

DECLARE_ENUM(zt_terminator_strategy, ZITI_TERMINATOR_STRATEGY)

DECLARE_MODEL(api_path, ZITI_API_PATH_MODEL)

DECLARE_MODEL(zt_api_versions, ZITI_API_VERSIONS_MODEL)

DECLARE_MODEL(zt_version, ZITI_VERSION_MODEL)

DECLARE_MODEL(zt_jwt_signer, ZITI_JWT_SIGNER_MODEL)

DECLARE_MODEL(zt_id_cfg, ZITI_ID_CFG_MODEL)

DECLARE_MODEL(zt_config, ZITI_CONFIG_MODEL)

DECLARE_MODEL(zt_identity, ZITI_IDENTITY_MODEL)

DECLARE_MODEL(zt_process, ZITI_PROCESS_MODEL)

DECLARE_MODEL(zt_posture_query, ZITI_POSTURE_QUERY_MODEL)

DECLARE_MODEL(zt_posture_query_set, ZITI_POSTURE_QUERY_SET_MODEL)

DECLARE_MODEL(zt_service, ZITI_SERVICE_MODEL)

DECLARE_MODEL(zt_terminator, ZITI_TERMINATOR_MODEL)

DECLARE_MODEL(zt_client_cfg_v1, ZITI_CLIENT_CFG_V1_MODEL)

DECLARE_MODEL(zt_port_range, ZITI_PORT_RANGE_MODEL)

DECLARE_MODEL(zt_intercept_cfg_v1, ZITI_INTERCEPT_CFG_V1_MODEL)

DECLARE_MODEL(zt_listen_options, ZITI_LISTEN_OPTS_MODEL)

DECLARE_MODEL(zt_server_cfg_v1, ZITI_SERVER_CFG_V1_MODEL)

DECLARE_ENUM(zt_proxy_server_type, ZITI_PROXY_SERVER_TYPE_ENUM)

DECLARE_MODEL(zt_proxy_server, ZITI_PROXY_SERVER_MODEL)

DECLARE_MODEL(zt_address_translation, ZITI_ADDRESS_TRANSLATION_MODEL)

DECLARE_MODEL(zt_host_cfg_v1, ZITI_HOST_CFG_V1_MODEL)

DECLARE_MODEL(zt_host_cfg_v2, ZITI_HOST_CFG_V2_MODEL)

DECLARE_MODEL(zt_mfa_enrollment, ZITI_MFA_ENROLLMENT_MODEL)

ZITI_FUNC model_bool zt_service_has_permission(const zt_service *service, zt_session_type sessionType);

ZITI_FUNC const char *zt_service_get_raw_config(zt_service *service, const char *cfg_type);

typedef int (*parse_service_cfg_f)(void *, const char *, size_t);

ZITI_FUNC int zt_service_get_config(zt_service *service, const char *cfg_type, void *cfg,
                                      parse_service_cfg_f parse_func);

ZITI_FUNC int zt_intercept_from_client_cfg(zt_intercept_cfg_v1 *intercept, const zt_client_cfg_v1 *client_cfg);

ZITI_FUNC int
zt_intercept_match(const zt_intercept_cfg_v1 *intercept, zt_protocol proto, const char *addr, int port);

ZITI_FUNC int zt_intercept_match2(const zt_intercept_cfg_v1 *intercept, zt_protocol proto, const zt_address *addr, int port);
