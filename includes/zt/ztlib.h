...
typedef uint32_t zt_handle_t;
#define ZITI_INVALID_HANDLE ((zt_handle_t)-1)

#if _WIN32
#include <WinSock2.h>
typedef SOCKET zt_socket_t;
#else
#include <netinet/in.h>

typedef int zt_socket_t;
#endif
 ...
 * Use [zt_errorstr] to get error message.
 * @return
 */
ZITI_FUNC
 ...
 *
 * @param h pointer to zt_handle_t to be initialized
 * @param identity identity config JSON or path to a file.
 * @return
 *   [ZITI_OK] success, returned handle can be used to access/bind zt services
 ...
 *
 * @return a dynamically allocated array of zt_jwt_signer pointers, terminated with NULL.
 */
ZITI_FUNC
const char * const *  Ziti_get_ext_signers(zt_handle_t ztx);
 ...
 * @param socket
 * @return 0 - not a zt socket, 1 - connected zt socket, 2 - zt server socket
 */
ZITI_FUNC
 ...
 */
ZITI_FUNC
int Ziti_connect(zt_socket_t socket, zt_handle_t ztx, const char *service, const char *terminator);
 ...
 */
ZITI_FUNC
int Ziti_bind(zt_socket_t socket, zt_handle_t ztx, const char *service, const char *terminator);
 ...
 *
 * Extracts the first [zt_connection] from pending queue, accepts it, and opens a new socket fd for it.
 *
