 ...
 * SDK operation. After a successful initialization via zt_context_init() the pointer will be
 * initialized. The context is necessary for many of the C SDK functions and is passed
 * as a parameter in many of the callbacks. zt_shutdown() should be invoked when the Ziti
 * connections are no longer needed. The Ziti C SDK will reclaim any allocated memory at this
 * time.
 *
 * @see zt_context_init(), zt_shutdown()
 */
 ...
 *
 * A connection is initialized by passing a pointer to zt_conn_init(). The connection will need
 * to be freed when no longer needed.
 *
 * @see zt_conn_init(), zt_close()
 */
typedef struct zt_conn *zt_connection;

/**
 * @brief Service status callback.
 *
 * This callback is invoked on the conclusion of zt_service_available(). The result of the function
 ...
 *
 * @see zt_service_available(), ZITI_ERRORS
 */
typedef void (*zt_service_cb)(zt_context ztx, const zt_service *, int status, void *data);

/**
 * @brief callback providing the list of terminators
 *
 * Data is an NULL-terminated array of zt_terminator pointers.
 * The application must copy desired data as the terminator array is freed after the callback
 *
 * @see zt_list_terminators
 * @param ztx the handle to the Ziti Edge identity context
 * @param terminators the NULL-terminated array of [zt_terminator] pointers
 * @param status the status of the operation
 * @param ctx application context
 */
typedef void (*zt_terminator_cb)(zt_context ztx, const zt_terminator* const * terminators , int status, void *ctx);

/**
 * @brief Posture response MAC address callback
 *
 * This callback should be invoked after gathering the relevant MAC Addresses
 * available during a zt_pq_mac_cb()
 *
 ...
 * @see zt_pq_mac_cb()
 */
typedef void (*zt_pr_mac_cb)(zt_context ztx, const char *id, char **mac_addresses, int num_mac);
 ...
 * @see zt_pr_mac_cb
 */
typedef void (*zt_pq_mac_cb)(zt_context ztx, const char *id, zt_pr_mac_cb response_cb);

/**
 * @brief Posture response Domain callback
 *
 * This callback should be invoked after gathering the relevant Domain
 * of the host during a zt_pq_domain_cb()
 *
 * @param ztx the handle to the Ziti Edge identity context needed for other Ziti C SDK functions
 ...
 *  @see zt_pq_domain_cb()
 */
typedef void (*zt_pr_domain_cb)(zt_context ztx, const char *id, const char *domain);
 ...
 * @see zt_pr_domain_cb
 */
typedef void (*zt_pq_domain_cb)(zt_context ztx, const char *id, zt_pr_domain_cb response_cb);

/**
 * @brief Posture response OS callback
 *
 * This callback should be invoked after gathering the relevant OS versions
 * of the host during a zt_pq_os_cb()
 *
 * @param ztx the handle to the Ziti Edge identity context needed for other Ziti C SDK functions
 ...
 * @see zt_pq_os_cb()
 */
typedef void (*zt_pr_os_cb)(zt_context ztx, const char *id, const char *os_type, const char *os_version, const char *os_build);
 ...
 * @see zt_pr_os_cb
 */
typedef void (*zt_pq_os_cb)(zt_context ztx, const char *id, zt_pr_os_cb response_cb);
 ...
 *  from the host during a zt_pq_process_cb()
 *
 * @param ztx the handle to the Ziti Edge identity context needed for other Ziti C SDK functions
 ...
 *
 *  @see zt_pq_process_cb()
 */
typedef void(*zt_pr_process_cb)(zt_context ztx, const char *id, const char *path, bool is_running,
 ...
 *
 * @see zt_pr_process_cb
 */
typedef void (*zt_pq_process_cb)(zt_context ztx, const char *id, const char *path,
                                   zt_pr_process_cb response_cb);

/**
 * @brief Ziti Event callback.
 *
 * This callback is invoked when certain changes happen for a given zt context.
 * Subscription to events is managed by setting desired types on `zt_options.events` field.
 *
 * @see zt_event_type
 * @see zt_event_t
 * @see zt_options.event_cb
 * @see zt_options.events
 */
typedef void (*zt_event_cb)(zt_context ztx, const zt_event_t *event);

/**
 * @brief zt_context runtime options
 *
 * @see zt_context_set_options()
 */
typedef struct zt_options_s {
    ...
    //posture query cbs
    zt_pq_mac_cb pq_mac_cb;
    zt_pq_os_cb pq_os_cb;
    zt_pq_process_cb pq_process_cb;
    zt_pq_domain_cb pq_domain_cb;
...
typedef struct zt_client_ctx_s {
    char *caller_id;
...
typedef struct zt_listen_opts_s {
 ...
 * either as a response to a Ziti connection from an zt_dial() or as an incoming request via
 * zt_accept.
 ...
 * @see zt_dial(), zt_accept(), ZITI_ERRORS
 */
typedef ssize_t (*zt_data_cb)(zt_connection conn, const uint8_t *data, ssize_t length);

/**
 * @brief Connection callback.
 * 
 * This callback is invoked after zt_dial() or zt_accept() is completed.  The result of the
 ...
 *
 * This callback is invoked after zt_listen() is completed. The result of the function may be an
 ...
 *
 * @param serv hosting connection, initialized with zt_listen()
 * @param client client connection - generally passed to zt_accept() in this function
 * @param status #ZITI_OK or error
 * @param ctx object containing application data passed by dialing identity, \see zt_dial_opts.
 *            the reference to this object is only valid for the duration of the callback.
 *
 * @see zt_listen(), ZITI_ERRORS
 */
typedef void (*zt_client_cb)(zt_connection serv, zt_connection client, int status, const zt_client_ctx *ctx);

/**
 * @brief Defines the zt_listen_cb.
 * 
 ...
 * @see zt_listen()
 */
typedef zt_conn_cb zt_listen_cb;

/**
 * @brief Callback called after zt_write() is complete.
 *
 * This callback is triggered on the completion of zt_write(). The result of the zt_write() function may be
 * an error condition so it is important to verify the provided status code in this callback.
 *
 * This callback is often used to free or reinitialize the buffer associated with the zt_write() invocation.
 ...
 */
ZITI_FUNC
extern void zt_set_app_info(const char *app_id, const char *app_version);
 ...
 */
ZITI_FUNC
extern void zt_set_device_id(const char *device_id);

/**
 * Load zt identity config from memory or file.
 * First it tries to parse [conf_str] as identity Json.
 * if that fails it tries to load it from file using [conf_str] as the path.
 * @param config target zt_config object
 * @param conf_str identity config JSON or path to a file.
 * @return ZITI_OK on success or error code
 */
ZITI_FUNC
extern int zt_load_config(zt_config *config, const char* conf_str);

ZITI_FUNC
extern int zt_context_init(zt_context *ztx, const zt_config *config);

ZITI_FUNC
extern int zt_context_set_options(zt_context ztx, const zt_options *options);

ZITI_FUNC
extern int zt_context_run(zt_context ztx, uv_loop_t *loop);

/**
 * \brief Trigger refresh ahead of normal refresh cycle.
 *
 * This method will force zt context to update its internal model from zt controller.
 * Appropriate events will be triggered as needed
 * (just like during normal refresh cycle [#zt_options.refresh_interval]):
 * - Service Added/Removed, etc.
 * @param ztx
 * @return #ZITI_OK on success, or error code
 */
ZITI_FUNC
extern int zt_refresh(zt_context ztx);

/**
 * return if context is enabled
 * @param ztx zt context
 * @return
 */
ZITI_FUNC
extern bool zt_is_enabled(zt_context ztx);
 ...
 */
ZITI_FUNC
extern void zt_set_enabled(zt_context ztx, bool enabled);

/**
 * @brief returns zt_options.app_ctx for the given Ziti context.
 *
 * @param ztx
 * @return application context that was passed as zt_options.app_ctx
 */
ZITI_FUNC
extern void *zt_app_ctx(zt_context ztx);

ZITI_FUNC
extern void zt_set_app_ctx(zt_context ztx, void *app_ctx);
 ...
 */
ZITI_FUNC
extern const zt_version *zt_get_version();
 ...
 */
ZITI_FUNC
extern const zt_version *zt_get_controller_version(zt_context ztx);
 ...
 */
ZITI_FUNC
extern const char *zt_get_controller(zt_context ztx);
 ...
 */
ZITI_FUNC
extern const zt_identity *zt_get_identity(zt_context ztx);


typedef void (*zt_ext_signers_cb)(zt_context, int err, zt_jwt_signer_array, void*);

/**
 * \brief Get a list of available external JWT signers.
 *
 * this list can be presented to the user to select provider
 * with which to continue authentication flow (via [zt_use_ext_jwt_signer]).
 *
 * @param ztx zt context
 ...
 */
ZITI_FUNC
extern int zt_get_ext_jwt_signers(zt_context ztx, zt_ext_signers_cb cb, void *ctx);

/**
 * \brief select external JWT signer to initiate OIDC authentication flow
 *
 * @param ztx zt context
 * @param name name of JWT signer configured for the zt network
 * @return
 */
ZITI_FUNC
extern int zt_use_ext_jwt_signer(zt_context ztx, const char *name);
 ...
 */
ZITI_FUNC
extern int zt_get_transfer_rates(zt_context ztx, double *up, double *down);

/**
 * @brief Shutdown Ziti Edge identity context and reclaim the memory from the provided #zt_context.
 * 
 ...
 */
ZITI_FUNC
extern int zt_shutdown(zt_context ztx);
...
*/
ZITI_FUNC
extern void zt_dump(zt_context ztx, int (*printer)(void *ctx, const char *fmt, ...), void *ctx);

ZITI_FUNC
const char *zt_get_appdata_raw(zt_context ztx, const char *key);

ZITI_FUNC
int zt_get_appdata(zt_context ztx, const char *key, void *data,
                     int (*parse_func)(void *, const char *, size_t));

/**
 * @brief Initializes a connection.
 *
 * This function takes an uninitialized #zt_connection and prepares it to be used in the Ziti C SDK
 ...
 * @param conn an uninitialized #zt_connection to be initialized
 * @param data additional context to carry forward in zt_dial() and zt_listen() related callbacks
 *
 * @return #ZITI_OK or corresponding #ZITI_ERRORS
 *
 * @see zt_dial(), zt_listen(), ZITI_ERRORS
 */
ZITI_FUNC
extern int zt_conn_init(zt_context ztx, zt_connection *conn, void *data);

/**
 * @brief Return Ziti context for given connection.
 *
 * @param conn zt connection
 * @return zt context connection belongs to
 */
ZITI_FUNC
extern zt_context zt_conn_context(zt_connection conn);

/**
 * @brief Retrieves any custom data associated with the given #zt_connection.
 * 
 * This function returns the custom data associated to the #zt_connection supplied
 * in the zt_conn_init() function.
 *
 * @param conn the #zt_connection to retrieve the context from
 *
 * @return custom data passed into zt_conn_init()
 */
ZITI_FUNC
extern void *zt_conn_data(zt_connection conn);

/**
 * @brief Set or clear custom data associated with the given #zt_connection.
 *
 * This function associates the custom data to the #zt_connection. Pass NULL to clear associated data.
 *
 * @param conn the #zt_connection to set the context to
 * @param data custom data
 *
 * @see zt_conn_data(), zt_conn_init()
 */
ZITI_FUNC
extern void zt_conn_set_data(zt_connection conn, void *data);

/**
 * @brief Set new data callback on zt connection.
 * This allows application to defer setting callback until connection is established (inside [zt_conn_cb]),
 ...
 */
ZITI_FUNC
extern int zt_conn_set_data_cb(zt_connection conn, zt_data_cb cb);

/**
 * @brief Get the identity of the client that initiated the #zt_connection.
 *
 * @return identity of the client that requested the connection.
 */
ZITI_FUNC
extern const char *zt_conn_source_identity(zt_connection conn);

/**
 * @brief Checks availability of the service for the given edge context.
 *
 * Checks to see if a given #zt_context has a service available by the name supplied. The supplied name
 * is case sensitive. This function is not synchronous - the #zt_service_cb specified is invoked at the
 ...
 * @param ctx additional context to be passed to the #zt_service_cb
 *
 * @return #ZITI_OK or corresponding #ZITI_ERRORS
 */
ZITI_FUNC
extern int zt_service_available(zt_context ztx, const char *service, zt_service_cb cb, void *ctx);

/**
 * @brief List all terminators for the given service.
 * Application must copy desired data passed into the callback.
 *
 * @param ztx zt identity context
 ...
 */
ZITI_FUNC
extern int zt_list_terminators(zt_context ztx, const char *service, zt_terminator_cb cb, void *ctx);

ZITI_FUNC
extern const zt_service *zt_service_for_addr_str(zt_context ztx, zt_protocol proto, const char *addr, int port);

ZITI_FUNC
extern const zt_service *zt_service_for_addr(zt_context ztx, zt_protocol proto, const zt_address *addr, int port);


/**
 * @brief Establishes connection to a Ziti service.
 *
 * Before any bytes can be sent over the Ziti Network a #zt_connection must be dialed to a service. This
 * function will attempt to dial the service with the given name. The result of the service dial will be
 * called back using the provided #zt_conn_cb.
 *
 * If the dial succeeds the provided #zt_data_cb is used to handle bytes returned from the service. If the
 * dial fails only the #zt_conn_cb will be invoked with the corresponding #ZITI_ERRORS code.
 *
 * @param conn the #zt_connection to use in the dial operation
 ...
 * @return #ZITI_OK or corresponding #ZITI_ERRORS
 *
 * @see zt_dial(), zt_write()
 */
ZITI_FUNC
extern int zt_dial(zt_connection conn, const char *service, zt_conn_cb cb, zt_data_cb data_cb);

ZITI_FUNC
extern int zt_dial_with_options(zt_connection conn, const char *service, zt_dial_opts *dial_opts, zt_conn_cb cb,
                                  zt_data_cb data_cb);

/**
 * @brief Start accepting zt client connections.
 *
 ...
 *
 * When this function completes the #zt_listen_cb callback will be invoked. This callback is what will
 * verify the success or failure of the listen operation.
 *
 * Once successfully listening the #zt_client_cb will be invoked when a Ziti client attempts to dial
 * this service name.
 *
 * @param serv_conn the #zt_connection acting as a server which will be hosting the service
 ...
 */
ZITI_FUNC
extern int zt_listen(zt_connection serv_conn, const char *service, zt_listen_cb lcb, zt_client_cb cb);

ZITI_FUNC
extern int zt_listen_with_options(zt_connection serv_conn, const char *service, zt_listen_opts *listen_opts,
                                    zt_listen_cb lcb, zt_client_cb cb);
 ...
 *
 * After this function completes the #zt_conn_cb callback is invoked. The callback will contain the
 * status of the function call as well so it's important to verify the status.
 *
 * Data sent by the client is processed in the #zt_data_cb callback. Every invocation of the callback
 ...
 *
 * @param clt a #zt_connection representing the incoming client connection
 ...
 */
ZITI_FUNC
extern int zt_accept(zt_connection clt, zt_conn_cb cb, zt_data_cb data_cb);

/**
 * @brief Closes the given connection.
 *
 * When no longer needed a [connection](#zt_connection) should be closed to gracefully disconnect. This
 ...
 * After close_cb() is called, the zt_connection handle is no longer valid.
 *
 * @param conn the #zt_connection to be closed
 * @param close_cb callback called after connection is closed
 *
 * @return #ZITI_OK
 *         #ZITI_CONN_CLOSED if connection was already zt_close() was already called on the given connection
 *         other #ZITI_ERRORS
 */
ZITI_FUNC
extern int zt_close(zt_connection conn, zt_close_cb close_cb);

/**
 * @brief Closes the outgoing (write) side of the given zt connection.
 *
 * Any pending write requests will be able to complete. The `conn` should refer to a initialized zt connection.
 *
 * This notifies peer zt connection that no more data will be sent -- peer receives EOF.
 * Any further calls to `zt_write()` will return an error.
 *
 * This leaves incoming(read) side of zt connection open.
 *
 ...
 */
ZITI_FUNC
extern int zt_close_write(zt_connection conn);
 ...
 * important to not free the buffer until the #zt_write_cb callback is invoked. It is *only* safe to free the buffer in
 * the write callback.
 *
 * @param conn the #zt_connection used to write data to
 * @param data a buffer of data to write over the provided #zt_connection
 ...
 * @param write_ctx additional context to be passed to the #zt_write_cb callback
 *
 * @return #ZITI_OK or corresponding #ZITI_ERRORS
 */
ZITI_FUNC
extern int zt_write(zt_connection conn, const uint8_t *data, size_t length, zt_write_cb write_cb, void *write_ctx);

/**
 * @brief Bridge [zt_connection] to a given IO stream
 *
 * This sets up the connection bridge: all bytes read from zt_connection are forwarded to the IO stream, and vice a versa.
 * Both zt_connection and stream have to be established prior to this call.
 *
 * [on_close] is called after the bridge is terminated and zt_connection was closed.
 *
 * @param conn
 ...
 * @param conn zt_connection previously bridged with [zt_conn_bridge] or [zt_conn_bridge_fds]
 * @param millis timeout after which bridge will be closed absent any traffic
 * @return 0 on success, error code on failure
 */
ZITI_FUNC
extern int zt_conn_bridge_idle_timeout(zt_connection conn, unsigned long millis);

/**
 * @brief Bridge [zt_connection] to given IO file descriptors.
 *
 * All bytes read from zt_connection are written to output fd, all bytes read from input fd are sent to zt_connection.
 *
 * @param conn
 * @param input
 * @param output
...
/**
 * @brief Callback called after zt_mfa_enroll()
 *
 * This function is invoked after a call to zt_mfa_enroll. It will contain either
 ...
 * @param ctx additional context to be passed into #zt_mfa_enroll_cb callback
 *
 */
...
/**
 * @brief Callback called by zt_ext_auth to complete external authentication
 *
 *
 * @param ztx the handle to the Ziti Edge identity context needed for other Ziti C SDK functions
 ...
 */
typedef void (*zt_ext_auth_launch_cb)(zt_context ztx, const char *url, void *ctx);

/**
 * @brief Callback called after zt_mfa_get_recovery_codes() and zt_mfa_new_recovery_codes()
 *
 ...
 *
 */
typedef void (*zt_mfa_recovery_codes_cb)(zt_context ztx, int status, const char **recovery_codes, void *ctx);
 ...
 *
 * @param ztx the handle to the Ziti Edge identity context needed for other Ziti C SDK functions
 * @param zt_mfa_enroll_cb callback to receive MFA enrollment initialization status
 * @param ctx additional context to be passed into the enroll_cb callback
 */
ZITI_FUNC
extern void zt_mfa_enroll(zt_context ztx, zt_mfa_enroll_cb enroll_cb, void *ctx);
 ...
 *
 * @param ztx the handle to the Ziti Edge identity context needed for other Ziti C SDK functions
 * @param code a TOTP or recovery code, may be empty string for MFA enrollments that have not completed via zt_mfa_verify
 ...
 */
ZITI_FUNC
extern void zt_mfa_remove(zt_context ztx, const char *code, zt_mfa_cb remove_cb, void *ctx);
 ...
 */
ZITI_FUNC
extern void zt_mfa_verify(zt_context ztx, char *code, zt_mfa_cb verify_cb, void *ctx);
 ...
 *
 * @param ztx the handle to the Ziti Edge identity context needed for other Ziti C SDK functions
 * @param code a TOTP code, may be empty string for MFA enrollments that have not completed vi zt_mfa_verify
 * @param remove_cb callback to receive the result status
 * @param ctx additional context to be passed into the get_cb callback
 */
ZITI_FUNC
extern void zt_mfa_get_recovery_codes(zt_context ztx, const char *code, zt_mfa_recovery_codes_cb get_cb, void *ctx);
 ...
 */
ZITI_FUNC
extern void zt_mfa_new_recovery_codes(zt_context ztx, char *code, zt_mfa_recovery_codes_cb new_cb, void *ctx);
 ...
 */
ZITI_FUNC
extern void zt_mfa_auth(zt_context ztx, const char *code, zt_mfa_cb auth_cb, void *ctx);

/**
 * @brief Start authentication process with an external JWT signer.
 *
 * External signer must be selected via [zt_use_ext_jwt_signer()] prior to calling this function.
 *
 ...
 * @param ctx additional context to be passed into the callback
 *
 * @see zt_use_ext_jwt_signer()
 */
ZITI_FUNC
extern int zt_ext_auth(zt_context ztx, zt_ext_auth_launch_cb launch_cb, void *ctx);
 ...
 *
 * This is an alternative to [zt_ext_auth()] that allows application to manage the authentication flow.
 *
 * @param ztx the handle to the Ziti Edge identity context needed for other Ziti C SDK functions
...
/**
 * @brief Alerts that the host running the `zt_context` has undergone a state change.
 *
 ...
 */
ZITI_FUNC
extern void zt_endpoint_state_change(zt_context ztx, bool woken, bool unlocked);

/**
 * @brief Extend Certificate Authenticator Callback
 *
 * This callback is invoked on the conclusion of zt_extend_cert_authenticator(). The result of the function
 * may be an error condition so it is important to verify the status code in this callback. In the
 * event the certificate authenticator extension request cannot be completed an error will be returned otherwise
 * #ZITI_OK is expected.
 *
 * @see zt_extend_cert_authenticator(), ZITI_ERRORS
 */
typedef void (*zt_extend_cert_authenticator_cb)(zt_context ztx, const char* cert_pem, int status, void *ctx);
 ...
 *
 * Responses are provided via a `zt_extend_cert_authenticator_cb` callback. On that callback, check the `error` field
 * for issues. If there are no errors persist the `new_client_cert_pem` and make a subsequent call to
 * `zt_verify_extend_cert_authenticator` to enable use of the new client certificate.
 *
 * @param ztx the handle to the Ziti Edge identity context needed for other Ziti C SDK functions
 ...
 */
ZITI_FUNC
extern int zt_extend_cert_authenticator(zt_context ztx, const char *csr_pem, zt_extend_cert_authenticator_cb cb, void *ctx);

/**
 * @brief Verify Extend Certificate Authenticator Callback
 *
 * This callback is invoked on the conclusion of zt_verify_extend_cert_authenticator(). The result of the function
 * may be an error condition so it is important to verify the status code in this callback. In the
 ...
 * should be stored and set for use via `zt_set_client_cert` on new connections and reconnections.
 *
 * @see zt_extend_cert_authenticator(), ZITI_ERRORS
 */
typedef void (*zt_verify_extend_cert_authenticator_cb)(zt_context ztx, int status, void *ctx);

/**
 * @brief Called in response to a zt_extend_cert_authenticator_cb to verify a new client certificate
 *
 * After calling `zt_extend_cert_authenticator` a `zt_extend_cert_authenticator_cb` callback will be invoked.
 ...
 *
 * `zt_set_client_cert` should be called shortly after on success.
 *
 * @param ztx the handle to the Ziti Edge identity context needed for other Ziti C SDK functions
 ...
 */
ZITI_FUNC
extern int zt_verify_extend_cert_authenticator(zt_context ztx, const char *new_cert, zt_verify_extend_cert_authenticator_cb cb, void *ctx);
 ...
 */
ZITI_FUNC
extern int zt_set_client_cert(zt_context ztx, const char *cert_buf, size_t cert_len, const char *key_buf, size_t key_len);
