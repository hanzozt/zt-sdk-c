...
const char *msg = rc > 0 ? strerror(err) : zt_errorstr(err); \
fprintf(stderr, desc"{" #op "} err=%d(%s)\n", err, msg); \
goto DONE;\
}                      \
} while(0)

static zt_socket_t non_blocking_accept(zt_socket_t srv, char *caller, int caller_len) {
#if _WIN32
    u_long opt = 1;
    ...
    do {
        ...
        zt_socket_t clt = Ziti_accept(srv, caller, caller_len);
...
int main(int argc, char *argv[]) {
    ...
    if (rc != 0) {
        fprintf(stderr, "failed to load zt context from %s: %s\n", argv[1], zt_errorstr(rc));
        exit(1);
    }

    zt_socket_t srv = socket(AF_INET, SOCK_STREAM, 0);
