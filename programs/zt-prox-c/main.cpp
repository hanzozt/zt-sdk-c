...
int main(int argc, char *argv[]) {
    ...
    CLI::App app{name};
    zt_set_app_info(name, zt_get_version()->revision);
    bool verbose = false;
    ...
    ver->final_callback([&verbose] {
        std::cout << zt_get_version()->version << std::endl;
        if (verbose) {
            std::cout << zt_get_version()->revision << std::endl
                    << zt_get_version()->build_date << std::endl;
        }
    });
