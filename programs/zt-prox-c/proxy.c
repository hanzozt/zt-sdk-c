...
void ext_auth_select(zt_context ztx, const zt_jwt_signer *providers[]);
...
void mfa_auth_event_handler(zt_context ztx);
void ext_auth_event_handler(zt_context ztx, const char *name);
...
static void close_binding_cb(zt_connection conn) {
    struct binding *b = zt_conn_data(conn);
...
static void process_stop(uv_loop_t *loop, struct proxy_app_ctx *app_ctx) {
    ...
    MODEL_MAP_FOR(it, app_ctx->bindings) {
        ...
        if (b->conn) {
            zt_close(b->conn, close_binding_cb);
        }
    }
...
static void debug_dump(struct proxy_app_ctx *app_ctx,
    ...
    zt_dump(app_ctx->zt, print_fn, printer);
}
...
static void reporter_cb(uv_timer_t *t) {
    ...
    if (app_ctx->zt != NULL) {
        zt_get_transfer_rates(app_ctx->zt, &up, &down);
...
static void signal_cb(uv_signal_t *s, int signum) {
    ...
    switch (signum) {
        ...
        case SIGUSR2: {
            ...
            zt_set_enabled(ctx->zt, !zt_is_enabled(ctx->zt));
            break;
        }
#endif
...
static void close_cb(uv_handle_t *h) {
    ...
    if (clt->zt_conn) {
        zt_conn_set_data(clt->zt_conn, NULL);
        zt_close(clt->zt_conn, NULL);
    }
...
static void on_zt_close(zt_connection conn) {
    uv_stream_t *tcp = zt_conn_data(conn);
    if (tcp) {
        struct client *clt = tcp->data;
        clt->zt_conn = NULL;
        ZITI_LOG(DEBUG, "zt connection closed for clt[%s]", clt->addr_s);
        clt->closed = true;
...
void on_zt_connect(zt_connection conn, int status) {
    uv_handle_t *clt = zt_conn_data(conn);

    if (status == ZITI_OK) {
        zt_conn_bridge(conn, clt, on_bridge_close);
    } else {
        ZITI_LOG(ERROR, "zt connect failed: %s(%d)", zt_errorstr(status), status);
        zt_close(conn, on_zt_close);
    }
}
...
static void on_client(uv_stream_t *server, int status) {
    ...
    PREPF(zt, zt_errorstr);
    TRY(zt, zt_conn_init(l->app_ctx->zt, &clt->zt_conn, c));
    zt_dial_opts opts = {
            .stream = true,
    };
    TRY(zt, zt_dial_with_options(clt->zt_conn, l->service_name, &opts, on_zt_connect, NULL));
    c->data = clt;
...
static void update_listener(zt_service *service, int status, struct listener *l) {
...
static void binding_listen_cb(zt_connection srv, int status) {
    struct binding *b = zt_conn_data(srv);
    if (status != ZITI_OK) {
        ZITI_LOG(WARN, "failed to bind to service[%s]", b->service_name);
        zt_close(b->conn, NULL);
        b->conn = NULL;
    }
}

static void on_zt_accept(zt_connection clt, int status) {
    uv_handle_t *s = zt_conn_data(clt);
    if (status == ZITI_OK) {
        if (zt_conn_bridge(clt, s, on_bridge_close) == 0) {
            if (s->type == UV_UDP) {
                zt_conn_bridge_idle_timeout(clt, 10000);
            }
        } else {
            ZITI_LOG(WARN, "failed to bridge zt connection and backend handle");
            zt_close(clt, NULL);
...
static void on_tcp_connect(uv_connect_t *conn_req, int status) {
    zt_connection clt = conn_req->data;

    if (status == 0) {
        zt_conn_set_data(clt, conn_req->handle);
        zt_accept(clt, on_zt_accept, NULL);
    } else {
...
static void binding_client_cb(zt_connection srv, zt_connection clt, int status, const zt_client_ctx *clt_ctx) {
    struct binding *b = zt_conn_data(srv);
    zt_context ztx = zt_conn_context(srv);
    struct proxy_app_ctx *pxy = zt_app_ctx(ztx);
    ...
    if (status == ZITI_OK) {
        ...
        ZITI_LOG(WARN, "stopping serving[%s] due to %d/%s", b->service_name, status, zt_errorstr(status));
        zt_close(b->conn, NULL);
        b->conn = NULL;
    }
}

static void service_check_cb(zt_context ztx, zt_service *service, int status, void *ctx) {
    struct proxy_app_ctx *app_ctx = ctx;
    ZITI_LOG(DEBUG, "service[%s]: %s", service->name, zt_errorstr(status));
    ...
    if (b && status == ZITI_OK && (service->perm_flags & ZITI_CAN_BIND) != 0) {
        if (b->conn == NULL) {
            zt_conn_init(ztx, &b->conn, b);
            zt_listen(b->conn, b->service_name, binding_listen_cb, binding_client_cb);
        }
    }
}

static void on_zt_event(zt_context ztx, const zt_event_t *event) {
    struct proxy_app_ctx *app_ctx = zt_app_ctx(ztx);
    switch (event->type) {
        case ZitiConfigEvent: {
            char *cfg = zt_config_to_json(event->cfg.config, 0, NULL);
            printf("new config:\n%s\n\n", cfg);
            free(cfg);
            break;
        }
        ...
        case ZitiContextEvent:
            if (event->ctx.ctrl_status == ZITI_OK) {
                const zt_version *ctrl_ver = zt_get_controller_version(ztx);
                const zt_identity *proxy_id = zt_get_identity(ztx);
        ...
        case ZitiServiceEvent:
            ...
            if (event->service.changed != NULL) {
                for (zt_service **sp = event->service.changed; *sp != NULL; sp++) {
                    zt_service *service = *sp;
                    service_check_cb(ztx, *sp, ZITI_OK, app_ctx);

                    MODEL_MAP_FOR(it, service->posture_query_map) {
                        zt_posture_query_set *policy = model_map_it_value(it);
                        for (int idx = 0; policy->posture_queries[idx] != NULL; idx++) {
                            zt_posture_query *query = policy->posture_queries[idx];

                            if (query->query_type == zt_posture_query_type_PC_MFA &&
        ...
        case ZitiRouterEvent:
            switch (event->router.status) {
                case EdgeRouterAdded:
                    ZITI_LOG(INFO, "zt added edge router %s address=%s", event->router.name,
                             event->router.address);
                    break;
                case EdgeRouterConnected:
                    ZITI_LOG(INFO, "zt connected to edge router %s, version = %s", event->router.name,
                             event->router.version);
                    break;
                case EdgeRouterDisconnected:
                    ZITI_LOG(INFO, "zt disconnected from edge router %s", event->router.name);
                    break;
                case EdgeRouterRemoved:
                    ZITI_LOG(INFO, "zt removed edge router %s", event->router.name);
                    break;
            ...
            break;
        case ZitiAuthEvent:
            if (event->auth.action == zt_auth_prompt_totp) {
                ZITI_LOG(INFO, "zt requires MFA %s/%s", event->auth.type, event->auth.detail);
                mfa_auth_event_handler(ztx);
            } else if (event->auth.action == zt_auth_login_external) {
                ext_auth_event_handler(ztx, NULL);
            } else if (event->auth.action == zt_auth_select_external) {
                ext_auth_select(ztx, (const zt_jwt_signer **)event->auth.providers);
            } else {
...
void mfa_response_cb(zt_context ztx, int status, void *ctx);
...
void mfa_worker_done(uv_work_t *req, int status) {
    ...
    if (status != 0) {
        ...
        zt_mfa_auth(mfa_wr->ztx, mfa_wr->code, mfa_response_cb, mfa_wr);
    }
    FREE(mfa_wr->code);
    FREE(mfa_wr);
}
...
static void ext_url_launch(zt_context ztx, const char *url, void *ctx) {
    char cmd[1024];
...
static void ext_auth_done(uv_work_t *wr, int status) {
    ...
    zt_ext_auth(pxy->zt, ext_url_launch, NULL);
    free(wr);
}
...
void ext_auth_event_handler(zt_context ztx, const char *name) {
    ...
    if (name) {
        ...
        zt_use_ext_jwt_signer(ztx, name);
        return;
    }
...
static void ext_auth_select_done(uv_work_t *wr, int status) {
    struct proxy_app_ctx *pxy = wr->data;
    zt_use_ext_jwt_signer(pxy->zt, cstr_str(&pxy->auth_provider));
    free(wr);
}
...
static void stopper_recv(uv_udp_t *u, ssize_t len,
    ...
    switch (cmd) {
        ...
        case ProxyCmd_enable:
            zt_set_enabled(app_ctx.zt, true);
            break;
        case ProxyCmd_refresh:
            zt_refresh(app_ctx.zt);
            break;
        case ProxyCmd_disable:
            zt_set_enabled(app_ctx.zt, false);
            break;
    }
}
...
int run_proxy(struct run_opts *opts) {
    ...
    zt_log_init(loop, opts->debug, NULL);
    ...
    zt_load_config(&cfg, opts->identity);
    zt_context_init(&app_ctx.zt, &cfg);
    free_zt_config(&cfg);
    ...
    zt_context_set_options(app_ctx.zt, &zopts);

    zt_context_run(app_ctx.zt, loop);
